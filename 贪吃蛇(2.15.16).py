...
# ========== 游戏版本：v2.15.16 - 修复ESC退出黑屏 ==========
VERSION = "v2.15.16"
CHANGELOG = f"""版本 {VERSION} - 开始游戏文字再下移1像素

✨ 新增功能：
- 仅“开始游戏”按钮文字再下移1像素（现中心下方17像素）
- 其他按钮文字保持不变（中心下方14像素）
- 悬停效果与点击区域完全匹配
...
kills = 0   # 新增：击杀计数
game_active = True
game_mode = "classic"  # 经典模式，固定值，用于发送给启动器
game_started = False
boosting = False
boost_start_time = 0
color_centers = {}
    ...
    if settings_mode:
        ...
        for i, (y, text, font_info) in enumerate(zip(button_y, button_texts, button_fonts)):
            ...
            if hover_button == i:
                ...
                writer.color("black")
            # 绘制文字：开始游戏按钮单独下移3像素（共17像素），其他保持14像素
            if i == 0:
                writer.goto(0, y - 17)  # 开始游戏按钮再下移1像素
            else:
                writer.goto(0, y - 14)  # 其他按钮不变
