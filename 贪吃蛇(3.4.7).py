...
settings_mode = False
paused = False
gameover_screen = False  # 新增：结算界面状态
pause_selection = 0
gameover_selection = 0   # 结算界面选中项
boosting = False
boost_start_time = 0
...
def draw():
    ...
    if settings_mode:
        ...
        # 如果游戏结束，显示结算界面（覆盖游戏画面）
        if not game_active:
            # 半透明遮罩
            overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
            overlay.fill((192, 255, 240, 220))  # 海沫绿半透明
            screen.blit(overlay, (0, 0))

            # 结算标题
            go_font = get_font(48)
            go_title = go_font.render("游戏结束", True, BLACK)
            screen.blit(go_title, (WIDTH//2 - go_title.get_width()//2, 200))

            # 显示最终得分
            score_font = get_font(32)
            score_display = score_font.render(f"最终得分: {score}", True, BLACK)
            ...
            hint = font_small.render("↑↓选择  Enter确认  鼠标点击选择", True, DARK_GRAY)
            screen.blit(hint, (WIDTH//2 - hint.get_width()//2, HEIGHT - 150))

        # 如果暂停，绘制暂停菜单（优先级高于结算？实际上不会同时出现）
        if paused:
            overlay = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                if game_started and not settings_mode:
                    if gameover_screen:
                        # 结算界面按ESC可忽略？可以返回开始界面或关闭结算？这里设为关闭结算并返回开始界面
                        gameover_screen = False
                        game_started = False
                        settings_mode = False
                    elif paused:
            ...
            if gameover_screen:
                ...
                for i in range(2):
                    ...
                    font = get_font(36)
                    rendered = font.render(opt_text, True, BLACK)
                    rect = pygame.Rect(WIDTH//2 - rendered.get_width()//2 - 10, option_y_start + i * 60 - 10,
                                       rendered.get_width() + 20, rendered.get_height() + 20)
                    if rect.collidepoint(mouse_x, mouse_y):
