...
# ========== 游戏版本：v2.15.2 - 修复悬停矩形位置 ==========
VERSION = "v2.15.2"
CHANGELOG = f"""版本 {VERSION} - 修复悬停矩形位置

✨ 新增功能：
- 开始界面鼠标悬停效果加强：按钮显示填充矩形，文字颜色加深
- 修复悬停矩形与点击区域不匹配的问题（统一尺寸和位置）
...
def on_mouse_move(event):
    ...
    if not game_started and not settings_mode:
        button_y_positions = [50, 0, -50, -100]  # 与draw中的y坐标对应
        hover_button = None
...
def on_click(event):
    ...
    x = event.x - WIDTH/2
    y = HEIGHT/2 - event.y
    button_y_positions = [50, 0, -50, -100]
    buttons = [
    ...
    for btn, y_pos in zip(buttons, button_y_positions):
