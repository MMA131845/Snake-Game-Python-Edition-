...
# 抗锯齿级别（可调节）
QUALITY_OPTIONS = ["性能", "均衡", "高品质"]
QUALITY_VALUES = [1, 2, 4]        # 对应 AA_SCALE
quality_index = 1                  # 默认均衡
AA_SCALE = QUALITY_VALUES[quality_index]
aa_surface = pygame.Surface((WIDTH // AA_SCALE, HEIGHT // AA_SCALE))
...
# 摸金模式参数
MOJIN_PHASE = "search"  # search, steal, withdraw
MOJIN_TIMER = 0
MOJIN_PHASE_DURATION = 300  # 帧数，约5秒（60fps）

# 全局攻击状态（仅淘汰之王模式）
global_attack_phase = False
attack_trigger_count = 15   # 增强：从30降到15

# 实用功能标志
show_fps = True
performance_mode = False  # 性能模式（关闭抗锯齿和阴影）— 已整合到画质选项，保留但不再使用
...
def get_enemy_new_direction(enemy, foods_list):
    ...
    # ========== 摸金模式专用逻辑 ==========
        ...
        if MOJIN_PHASE == "search":
            # 搜索阶段：分散寻找玩家，但保持距离
            target_x = player_head[0] + random.randint(-200, 200)
            target_y = player_head[1] + random.randint(-200, 200)
        elif MOJIN_PHASE == "steal":
            # 摸金阶段：全力冲向玩家预测位置前方
        ...
        if length > 1e-6:
            ...
            if direction_will_collide(head, desired_dir, all_segments, radius):
                # 尝试绕行
                for angle_offset in [-0.6, -0.3, 0.3, 0.6]:
                    cos_a = math.cos(angle_offset)
                    sin_a = math.sin(angle_offset)
                    test_x = desired_dir[0] * cos_a - desired_dir[1] * sin_a
                    test_y = desired_dir[0] * sin_a + desired_dir[1] * cos_a
...
def draw_circle(surface, x, y, color, radius):
    """绘制圆，支持抗锯齿"""
    sx = int((x - cam_x + WIDTH // 2) / AA_SCALE)
    sy = int((y - cam_y + HEIGHT // 2) / AA_SCALE)
    r = int(radius / AA_SCALE)
    pygame.draw.circle(surface, color, (sx, sy), r)

def draw_shadow(surface, x, y, radius):
    """绘制阴影"""
    sx = int((x - cam_x + WIDTH // 2) / AA_SCALE) + 2 // AA_SCALE
    sy = int((y - cam_y + HEIGHT // 2) / AA_SCALE) + 2 // AA_SCALE
    r = int(radius / AA_SCALE)
    pygame.draw.circle(surface, (0,0,0), (sx, sy), r)

def draw_background():
    """绘制背景（支持纯色、网格、星空）"""
    global background_style
    bg_value = BACKGROUND_STYLE_VALUES[background_style]
    if bg_value == BLACK:
        aa_surface.fill(BLACK)
    elif bg_value == WHITE:
        aa_surface.fill(WHITE)
    elif bg_value == "grid":
        aa_surface.fill(WHITE)
        ...
        while x < end_x:
            ...
            while y < end_y:
                ...
                sx = int((x - cam_x + WIDTH // 2) / AA_SCALE)
                sy = int((y - cam_y + HEIGHT // 2) / AA_SCALE)
                pygame.draw.rect(aa_surface, color, (sx, sy, GRID_SIZE//AA_SCALE, GRID_SIZE//AA_SCALE))
                y += GRID_SIZE
            x += GRID_SIZE
    elif bg_value == "stars":
        aa_surface.fill(BLACK)
        for _ in range(100):
            sx = random.randint(0, WIDTH // AA_SCALE)
            sy = random.randint(0, HEIGHT // AA_SCALE)
            brightness = random.randint(100, 255)
            aa_surface.set_at((sx, sy), (brightness, brightness, brightness))
...
def draw():
    ...
    global aa_surface, AA_SCALE, quality_index
    ...
    if settings_mode:
        ...
        # 设置选项（新增画质）
        items = [
            f"画质: {QUALITY_OPTIONS[quality_index]}",
    ...
    # ========== 游戏进行中 ==========
        ...
        for i, (x, y) in enumerate(snake):
            draw_shadow(aa_surface, x, y, current_radius)
        for enemy in enemies:
            for i, (x, y) in enumerate(enemy['body']):
                draw_shadow(aa_surface, x, y, current_radius)
        for fx, fy, fcolor, fr in foods:
            draw_shadow(aa_surface, fx, fy, fr)

        # 绘制主体
        for i, (x, y) in enumerate(snake):
            color = head_color if i == 0 else body_color
            draw_circle(aa_surface, x, y, color, current_radius)
        for enemy in enemies:
            for i, (x, y) in enumerate(enemy['body']):
                color = RED if i == 0 else DARK_RED
                draw_circle(aa_surface, x, y, color, current_radius)
        for fx, fy, fcolor, fr in foods:
            draw_circle(aa_surface, fx, fy, fcolor, fr)
        ...
        pygame.draw.rect(aa_surface, RED, (left//AA_SCALE, bottom//AA_SCALE, (right-left)//AA_SCALE, (top-bottom)//AA_SCALE), 2//AA_SCALE)

        # 缩放抗锯齿表面到屏幕
        pygame.transform.scale(aa_surface, (WIDTH, HEIGHT), screen)
...
def restart_game():
    global snake, direction, foods, score, game_active, boosting, color_centers, enemies, gameover_killer_name, global_attack_phase, MOJIN_PHASE, MOJIN_TIMER
    snake = [(0, 0), (-16, 0), (-32, 0), (-48, 0), (-64, 0)]
    direction = (1, 0)
    score = 0
    game_active = True
...
def new_game():
    restart_game()  # 与 restart_game 相同
...
def settings_left():
    global background_style, head_color_index, body_color_index, quality_index
    if not settings_mode:
        return
    if selected_setting == 0:  # 画质
        quality_index = (quality_index - 1) % len(QUALITY_OPTIONS)
    elif selected_setting == 1:  # 背景样式
        background_style = (background_style - 1) % len(BACKGROUND_STYLE_OPTIONS)
    elif selected_setting == 2:  # 蛇头颜色
        head_color_index = (head_color_index - 1) % len(HEAD_COLOR_OPTIONS)
    elif selected_setting == 3:  # 蛇身颜色
...
def settings_save():
    global settings_mode, AA_SCALE, aa_surface
    # 应用画质设置
    AA_SCALE = QUALITY_VALUES[quality_index]
    # 重新创建 aa_surface
    global aa_surface
...
def settings_cancel():
    global settings_mode, background_style, head_color_index, body_color_index, quality_index, AA_SCALE, aa_surface
    # 恢复原始设置（从全局默认值）
    background_style = DEFAULT_BACKGROUND_STYLE
...
def enter_settings():
    global settings_mode, selected_setting, quality_index_bak
    if not game_started and not settings_mode:
        settings_mode = True
        selected_setting = 0
        # 备份当前画质索引以便取消时恢复
        global quality_index_bak
        quality_index_bak = quality_index
...
MOJIN_PHASE = "search"
MOJIN_TIMER = 0
quality_index_bak = quality_index  # 用于取消
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.key == pygame.K_p:  # 按P切换性能模式（保留但不再使用，可移除）
                pass
            if paused:
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            if game_mode == "timed" and enemy['score'] >= 20 and enemy.get('state') == 'attacking':
                enemy_speed = get_base_speed() * 4.0
            elif game_mode == "mojin" and MOJIN_PHASE == "steal":
                enemy_speed = get_base_speed() * 2.5
            else:
