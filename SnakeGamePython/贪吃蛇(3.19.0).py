...
def get_enemy_new_direction(enemy, foods_list):
    """
    根据游戏模式决定行为：
    - 淘汰之王模式：只有当分数≥20的敌人数量 > 20时，分数≥20的敌人才会攻击玩家（协作包抄）；
      否则（包括分数<20或高分数量不足）都执行躲避行为。
    ...
    if should_avoid:
        ...
        if game_mode == "timed":  # 淘汰之王：协作包抄攻击
            player_head = snake[0]
            player_dir = direction
            ahead_dist = 100
            # 基础目标点：玩家正前方
            base_target = (player_head[0] + player_dir[0] * ahead_dist,
                           player_head[1] + player_dir[1] * ahead_dist)

            # 计算从玩家指向AI的向量
            to_ai_x = head[0] - player_head[0]
            to_ai_y = head[1] - player_head[1]
            # 计算玩家方向的垂直向量（左转90度）
            perp_x = -player_dir[1]
            perp_y = player_dir[0]

            # 计算AI相对于玩家方向的位置（点积判断前后，叉积判断左右）
            dot = to_ai_x * player_dir[0] + to_ai_y * player_dir[1]
            cross = to_ai_x * perp_x + to_ai_y * perp_y  # 等价于二维叉积

            # 偏移量，根据AI在玩家的左侧或右侧决定正负
            offset_magnitude = 50
            if abs(dot) > 0.1:
                if cross > 0:  # AI在玩家左侧
                ...
                # AI正好在玩家正前方，随机选择一个方向避免扎堆
                offset = offset_magnitude if random.random() > 0.5 else -offset_magnitude
