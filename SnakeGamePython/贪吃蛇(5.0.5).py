...
set_world_size(5000, 4000)  # 默认世界大小
...
base_red_hp = BASE_HP
base_blue_pos = (-200 * SCALE, 0)
base_red_pos = (200 * SCALE, 0)
respawn_timers = {}
...
def restart_game():
    ...
    global base_blue_pos, base_red_pos
    snake = [(0,0), (-16*SCALE,0), (-32*SCALE,0), (-48*SCALE,0), (-64*SCALE,0)]
    direction = (1,0)
    score = 0
    game_active = True
    ...
    if game_mode == "team4v4":
        set_world_size(1000, 800)      # 团队地图 1000x800
        base_blue_pos = (-250 * SCALE, 0)
        base_red_pos = (250 * SCALE, 0)
        snake = [(base_blue_pos[0], base_blue_pos[1]),
                 (base_blue_pos[0]-16*SCALE, base_blue_pos[1]),
                 (base_blue_pos[0]-32*SCALE, base_blue_pos[1])]
        for i in range(3):
            x = base_blue_pos[0] + random.randint(-int(80*SCALE), int(80*SCALE))
            y = base_blue_pos[1] + random.randint(-int(80*SCALE), int(80*SCALE))
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
            teammates.append({'body': body, 'dir': (1,0), 'team': 'blue', 'name': f"队友{i+1}"})
        for i in range(4):
            x = base_red_pos[0] + random.randint(-int(80*SCALE), int(80*SCALE))
            y = base_red_pos[1] + random.randint(-int(80*SCALE), int(80*SCALE))
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
