...
ALL_CHANGELOGS = [
    ("v3.7.0", [
        "时间挑战模式改为固定敌人数量，敌人死亡后不再重生",
        "开局所有敌人同时生成，存活到时间结束即为胜利",
    ]),
    ("v3.6.0", [
        "新增时间挑战模式（限时生存，得分倍率递增）",
        "增加排行榜功能（仅限时间挑战模式）",
        "开始界面可切换模式（M键）并查看排行榜（H键）",
    ]),
    ("v3.5.0", [
        "AI会躲避玩家，优先级比吃食物高",
    ]),
...
ENEMY_COUNT = 3  # 固定敌人数量
MAX_ENEMIES = 10  # 不再使用，保留但无意义
ENEMY_SPAWN_DELAY = 2000  # 不再使用
ENEMY_SPAWN_INTERVAL = 5  # 不再使用
FOOD_PER_SEGMENT = 3
...
enemies = []  # 敌人列表，固定数量，死亡后不移除，而是保留（但不再移动？这里我们依然移除，因为死亡即消失）
foods = []
score = 0
game_active = True
game_started = False
...
time_left = 60              # 计时模式剩余时间（秒）
score_multiplier = 1         # 当前得分倍率
score_recorded = False       # 是否已记录分数到排行榜
...
# 移除 spawn_enemies 和 periodic_enemy_spawn 函数

def get_enemy_new_direction(enemy, foods_list):
    """敌人AI决策：优先躲避玩家（距离小于8倍半径），其次寻找食物，最后随机"""
    old_dir = enemy['dir']
    head = enemy['body'][0]
    player_head = snake[0]

    dx_player = head[0] - player_head[0]
    dy_player = head[1] - player_head[1]
    dist_sq = dx_player*dx_player + dy_player*dy_player
    avoid_threshold_sq = (get_segment_radius() * 8) ** 2

    if dist_sq < avoid_threshold_sq:
        length = math.hypot(dx_player, dy_player)
        if length > 0:
            dx = dx_player / length
            dy = dy_player / length
...
def show_rankings():
    ...
    title = font_title.render("时间挑战排行榜", True, BLACK)
...
def draw():
    ...
    if settings_mode:
        ...
        mode_text = f"模式: {'时间挑战' if game_mode == 'timed' else '经典'}  (M键切换)"
        ...
        if mode_hover:
            pygame.draw.rect(screen, LIGHT_GRAY, mode_rect)
            pygame.draw.rect(screen, GRAY, mode_rect, 2)
        ...
        enemy_count = font_small.render(f"敌人: {len(enemies)}/{ENEMY_COUNT}", True, text_color)
        screen.blit(enemy_count, (20, 70))

        # 计时模式额外显示
        if game_mode == "timed" and game_started and not gameover_screen:
            time_color = YELLOW if time_left < 10 else text_color
            time_text = font_small.render(f"时间: {int(time_left)}s", True, time_color)
            screen.blit(time_text, (20, 90))
            mult_text = font_small.render(f"倍率: x{score_multiplier}", True, text_color)
            screen.blit(mult_text, (20, 110))
        ...
        if not game_active:
            ...
            # 显示最高倍率（计时模式）
            if game_mode == "timed":
                max_mult = score_multiplier  # 结束时就是最高倍率
                mult_display = font_info.render(f"最高倍率: x{max_mult}", True, WHITE)
                screen.blit(mult_display, (WIDTH//2 - mult_display.get_width()//2, 350))

            option_font = get_font(36)
            option_y_start = 400 if game_mode == "classic" else 450
...
def restart_game():
    ...
    global snake, direction, foods, score, game_active, boosting, color_centers, enemies, time_left, score_multiplier, score_recorded
    snake = [(0, 0), (-16, 0), (-32, 0), (-48, 0), (-64, 0)]
    direction = (1, 0)
    score = 0
    game_active = True
...
def start_game():
    global game_started, settings_mode, time_left, score_multiplier, score_recorded
        ...
        if event.type == pygame.QUIT:
            ...
            if not game_started and not settings_mode:
                ...
                for i, y_center in enumerate(BUTTON_Y_POS):
                    ...
                    if (screen_y - BUTTON_HALF_HEIGHT <= mouse_y <= screen_y + BUTTON_HALF_HEIGHT and
                        ...
                        hover_button = i
                        break
                # 模式按钮悬停已由 draw 中的 mode_hover 更新
            else:
                hover_button = None
            ...
            if event.button == 1:
                if not game_started and not settings_mode:
                    ...
                    # 模式按钮
                    mode_font = get_font(24)
                    mode_text = f"模式: {'时间挑战' if game_mode == 'timed' else '经典'}"
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        if game_mode == "timed":
            ...
            if time_left <= 0:
                ...
                time_left = 0
                if not score_recorded and score > 0:
                    add_score("玩家", score)
                    score_recorded = True
            else:
                # 更新倍率（每10秒一级）
        ...
        if check_boundary(new_head):
            game_active = False
            gameover_screen = True
            if not score_recorded and game_mode == "timed" and score > 0:
                add_score("玩家", score)
                score_recorded = True
            draw()
            pygame.display.flip()
