...
NAME_FILE = os.path.join(os.path.dirname(__file__), "player_name.txt")  # 玩家名称存储文件
...
# ========== 玩家名称（从文件加载） ==========
player_name = load_player_name()  # 如果文件不存在则为None
if player_name is None:
    player_name = "玩家"  # 默认名称，但会在首次启动界面被修改
...
def show_name_input_screen():
    ...
    while waiting:
        ...
        text_surface = font_input.render(display_text, True, BLACK)
...
if player_name == "玩家" and not os.path.exists(NAME_FILE):
    # 注意：上面加载时如果文件不存在，player_name 被设为 "玩家"，我们需要判断文件是否存在
    # 实际上我们在加载时已经检查了文件是否存在，如果不存在返回None，然后赋值为"玩家"
    # 所以这里可以通过检查文件是否存在来判断是否是首次运行
    # 如果文件不存在，显示输入界面
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            if game_mode == "timed" and enemy['score'] >= 20:
                high_score_count = sum(1 for e in enemies if e['score'] >= 20)
                if high_score_count > 20:
                    enemy_speed = get_base_speed() * 2.0
                else:
