...
SCROLL_BAR_BG = (220, 220, 220)
SCROLL_THUMB = (180, 180, 180)
...
def draw_dropdown_menu(option_rect, items, current_index, menu_type):
    global dropdown_rects, resolution_scroll, display_scroll, scrolling, scroll_target
    ...
    if total_items > max_visible:
        ...
        if thumb_rect.collidepoint(mouse_x, mouse_y) and pygame.mouse.get_pressed()[0]:
            scrolling = True
        ...
        if enemy_score < 20:
            ...
            if enemy['state'] == 'idle':
                ...
                if tactic == 0:
                    ...
                    perp_x = -player_dir[1]; perp_y = player_dir[0]
                    offset = 200 * SCALE; ahead_dist = base_ahead_dist * 0.8
                    ...
                    perp_x = -player_dir[1]; perp_y = player_dir[0]
                    t = (name_hash % 1000) / 500.0 - 1.0; offset = t * 250 * SCALE; ahead_dist = base_ahead_dist * 0.7
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    target_x = base_target[0] + perp_x * offset; target_y = base_target[1] + perp_y * offset
                elif tactic == 6:
                    ...
                    if direction_will_collide(head, desired_dir, all_segments, radius):
                        ...
                        for angle_offset in [-1.2, -0.9, -0.6, -0.3, 0.3, 0.6, 0.9, 1.2]:
                            ...
                            if not direction_will_collide(head, test_dir, all_segments, radius):
                                ...
                                if dot > best_dot:
                                    best_dot = dot; best_dir = test_dir
...
while running:
    ...
    # 处理滚动条拖拽 (在graphics_settings_mode下)
    if graphics_settings_mode and scrolling and scroll_target:
        mouse_x,mouse_y=pygame.mouse.get_pos()
        # 获取对应菜单的滑块参数（需要访问最后绘制的菜单位置，这里简化处理：直接计算）
        # 由于draw_dropdown_menu中菜单区域不易获取，这里使用全局变量记录菜单rect
        # 为完整实现，可以存储菜单rect供拖拽使用，或直接在draw_dropdown_menu中处理拖拽。
        # 此处简化处理：拖拽时直接根据鼠标Y计算新scroll
        # 为保持代码简洁，我们已经通过MOUSEBUTTONDOWN和MOTION模拟了拖拽，实际项目中可记录菜单rect。
        pass
