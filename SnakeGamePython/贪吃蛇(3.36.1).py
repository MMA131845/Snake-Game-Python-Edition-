...
def is_overlap(x, y, radius, lst):
    """检查位置是否与列表中的元素重叠，lst 中元素可以包含多于3个值"""
    for item in lst:
        ...
        if event.type == pygame.QUIT: running = False
            ...
            if graphics_settings_mode:
                ...
                if res_rect.collidepoint(mouse_x, mouse_y):
                    graphics_selected = 0; resolution_dropdown = not resolution_dropdown; display_dropdown = theme_dropdown = False
                elif disp_rect.collidepoint(mouse_x, mouse_y):
                    graphics_selected = 1; display_dropdown = not display_dropdown; resolution_dropdown = theme_dropdown = False
                elif theme_rect.collidepoint(mouse_x, mouse_y):
                    graphics_selected = 2; theme_dropdown = not theme_dropdown; resolution_dropdown = display_dropdown = False
                else:
                    # 下拉菜单项点击
                    ...
                    for menu_type, info in scroll_info.items():
                        if info['thumb_rect'].collidepoint(mouse_x, mouse_y):
                            scrolling = True; scroll_target = menu_type; break
                continue
            elif settings_mode:
                back_font = get_font(24); back_text = back_font.render("返回", True, BLACK)
                text_rect = back_text.get_rect(center=(BACK_BUTTON_POS[0]+50, BACK_BUTTON_POS[1]+20))
