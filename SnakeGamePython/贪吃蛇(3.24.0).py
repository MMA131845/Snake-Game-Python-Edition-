...
ALL_CHANGELOGS = [
    ("v3.24.0", [
        "新增玩家名字输入，排行榜和淘汰信息显示玩家名字",
        "淘汰之王模式排名显示玩家在所有敌人中的名次（如 3/101）",
    ]),
    ("v3.23.0", [
...
# ========== 玩家名字（新增） ==========
player_name = ""  # 玩家名字，默认为空，需要输入
input_name_active = False  # 是否正在输入名字
input_name_text = ""  # 当前输入的文本

# ========== 玩家轨迹记录（用于AI预测） ==========
player_pos_history = deque(maxlen=60)  # 记录最近1秒的位置（60帧）
player_dir_history = deque(maxlen=60)  # 记录最近1秒的方向
...
def get_enemy_new_direction(enemy, foods_list):
    """
    根据游戏模式决定行为：
    - 淘汰之王模式：只有当分数≥20的敌人数量 > 20时，分数≥20的敌人才会攻击玩家；
      攻击方式：战术角色分配（前锋/侧翼/后卫）、玩家轨迹预测、动态包围圈、避开蛇身。
    ...
    else:  # 攻击模式（根据模式选择具体攻击行为）
        if game_mode == "timed":  # 淘汰之王：战术攻击
            player_head = snake[0]
            ...
            role = name_hash % 3  # 0:前锋, 1:侧翼, 2:后卫

            # 基础绕前距离：100 + 玩家速度 * 10
            base_ahead_dist = 100 + player_speed * 10

            # 根据角色调整目标点
            if role == 0:  # 前锋：正面拦截，瞄准玩家正前方
            ...
            elif role == 1:  # 侧翼：从两侧包抄，偏移宽度与敌人数量相关
                spread_width = 200 + len([e for e in enemies if e['score'] >= 20]) * 5  # 动态宽度
                # 利用名字哈希生成偏移方向
                t = (name_hash % 1000) / 500.0 - 1.0  # [-1,1]
                offset = t * spread_width
                perp_x = -player_dir[1]
                perp_y = player_dir[0]
                ahead_dist = base_ahead_dist * 0.8  # 侧翼稍近一些
            ...
            if length > 1e-6:
                ideal_dir = (dx / length, dy / length)

                # --- 避开蛇身逻辑开始 ---
                # 检查理想方向是否会与任何蛇身碰撞
                radius = get_segment_radius()
                if direction_will_collide(head, ideal_dir, all_segments, radius, check_dist=60):
                    # 尝试多个偏移方向（从-0.6到0.6弧度，步长0.2）
                    best_dir = None
                    best_dot = -float('inf')
                    for angle_offset in [-0.6, -0.4, -0.2, 0.2, 0.4, 0.6]:
                        # 旋转向量
                        cos_a = math.cos(angle_offset)
                        sin_a = math.sin(angle_offset)
                        test_x = ideal_dir[0] * cos_a - ideal_dir[1] * sin_a
                        test_y = ideal_dir[0] * sin_a + ideal_dir[1] * cos_a
                        test_dir = (test_x, test_y)
                        # 检查是否会掉头
                        if test_dir[0] == -old_dir[0] and test_dir[1] == -old_dir[1]:
                            continue
                        # 检查是否碰撞
                        if not direction_will_collide(head, test_dir, all_segments, radius, check_dist=60):
                            # 计算与理想方向的一致程度（点积）
                            dot = test_dir[0] * ideal_dir[0] + test_dir[1] * ideal_dir[1]
                            if dot > best_dot:
                                best_dot = dot
                                best_dir = test_dir
                    if best_dir is not None:
                        ...
                        # 无安全方向，暂时躲避（但不返回，继续尝试吃食物或随机）
                        pass
                else:
                    # 理想方向安全
                    if not (ideal_dir[0] == -old_dir[0] and ideal_dir[1] == -old_dir[1]):
                        return ideal_dir

        else:  # 经典模式：追击最近的敌人（保持不变）
            target_enemy = None
...
# ========== 排行榜相关函数（保存所有分数） ==========
def load_scores():
    """加载所有历史分数，返回按得分降序排列的列表，每个元素为 [name, score]"""
...
def get_current_rank():
    """计算玩家在本局所有敌人中的排名（1-based）"""
    # 收集所有分数：玩家分数 + 所有敌人分数
    all_scores = [score] + [e['score'] for e in enemies]
    # 降序排序
    all_scores.sort(reverse=True)
    # 找到玩家分数所在位置（第一个等于玩家分数的位置，如果多个相同分数，玩家可能排在前或后，我们按列表顺序取第一个出现的位置）
...
def draw():
    global setting_option_rects, back_button_hover, mode_hover, gameover_killer_name, input_name_active, input_name_text
    screen.fill(BLACK)
    ...
    if settings_mode:
        ...
        name_prompt = name_font.render("你的名字:", True, BLACK)
        screen.blit(name_prompt, (WIDTH//2 - 200, HEIGHT//2 - 100))

        # 输入框背景
        input_rect = pygame.Rect(WIDTH//2 - 100, HEIGHT//2 - 100, 300, 40)
        pygame.draw.rect(screen, WHITE, input_rect)
        pygame.draw.rect(screen, DARK_GRAY, input_rect, 2)

        # 显示当前输入文本
        if input_name_active:
            display_text = input_name_text + "|"  # 光标效果
        else:
            display_text = input_name_text if input_name_text else "点击输入"
        name_surface = name_font.render(display_text, True, BLACK)
        ...
        if game_mode == "timed":
            high_score_count = sum(1 for e in enemies if e['score'] >= 20)
            rage_color = RED if high_score_count > 20 else text_color
            rage_text = font_small.render(f"狂暴敌人: {high_score_count}/20", True, rage_color)
            screen.blit(rage_text, (20, 90))

        # 显示本局排名（仅淘汰之王模式）
        if game_mode == "timed":
            current_rank = get_current_rank()
            total_entities = len(enemies) + 1
            rank_text = font_small.render(f"排名: {current_rank}/{total_entities}", True, text_color)
            screen.blit(rank_text, (20, 110))
        ...
        if not game_active:
            ...
            if game_mode == "timed" and gameover_killer_name:
                current_rank = get_current_rank()
                total_entities = len(enemies) + 1
                rank_text = f"本局排名 {current_rank}/{total_entities}"
...
def restart_game():
    ...
    global snake, direction, foods, score, game_active, boosting, color_centers, enemies, gameover_killer_name
    snake = [(0, 0), (-16, 0), (-32, 0), (-48, 0), (-64, 0)]
    direction = (1, 0)
    score = 0
    game_active = True
        ...
        for attempt in range(500):
            ...
            if not overlap:
                ...
                if game_mode == "timed":
                    ...
                    name = "Enemy"
                enemies.append({'body': body, 'dir': dir, 'score': 0, 'name': name})
                placed = True
                break
        if not placed:
            x = random.randint(WORLD_MIN_X + 100, WORLD_MAX_X - 100)
...
def start_game():
    global game_started, settings_mode, player_name, input_name_text
    # 检查玩家名字是否已设置
    if not player_name:
        # 如果输入框有文本，使用它
        if input_name_text.strip():
            player_name = input_name_text.strip()
        else:
            # 默认名字
            player_name = "玩家"
...
last_time = time.time()
running = True
gameover_killer_name = None  # 记录淘汰者的名字
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # 处理名字输入
            if not game_started and not settings_mode and input_name_active:
                if event.key == pygame.K_RETURN:
                    # 回车确认输入
                    input_name_active = False
                    player_name = input_name_text.strip() if input_name_text.strip() else "玩家"
                elif event.key == pygame.K_BACKSPACE:
                    input_name_text = input_name_text[:-1]
                elif event.key == pygame.K_ESCAPE:
                    input_name_active = False  # 取消输入
                else:
                    # 只允许字母数字和常用符号，限制长度
                    if len(input_name_text) < 15 and event.unicode.isprintable():
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            # 计算敌人速度：淘汰之王且分数≥20且高分总数>20时加速3倍，否则为基础速度
            if game_mode == "timed" and enemy['score'] >= 20:
                high_score_count = sum(1 for e in enemies if e['score'] >= 20)
                if high_score_count > 20:
                    enemy_speed = get_base_speed() * 3.0
                else:
        ...
        for enemy in enemies:
            for segment in enemy['body'][1:]:
                if (player_head[0] - segment[0]) ** 2 + (player_head[1] - segment[1]) ** 2 < collision_threshold_sq:
                    game_active = False
                    gameover_screen = True
                    gameover_killer_name = enemy['name']  # 记录淘汰者名字
                    if game_mode == "timed" and score > 0:
                        add_score(player_name, score)
                    break
            if not game_active:
            ...
            for _ in range(5):  # 每帧最多尝试5次，避免死循环
                ...
                for attempt in range(50):  # 每次尝试最多50次随机位置
                    ...
                    if not overlap:
                        ...
                        dir_options = [(1,0), (-1,0), (0,1), (0,-1)]
                        dir = random.choice(dir_options)
                        enemies.append({'body': body, 'dir': dir, 'score': 0, 'name': "Enemy"})
                        placed = True
                        break
                if not placed:
