...
ALL_CHANGELOGS = [
    ...
    ("v3.30.2", ["字体强制使用微软雅黑，提高中文显示兼容性"]),
    ("v3.30.1", ["修复中文字体加载问题，增强字体兼容性"]),
    ("v3.30.0", [
...
# Win11 风格下拉菜单专用颜色
MENU_BG = (250, 250, 250)           # 菜单背景
MENU_BORDER = (210, 210, 210)       # 菜单边框
MENU_HOVER = (230, 240, 255)        # 鼠标悬停高亮（浅蓝）
MENU_HOVER_BORDER = (150, 190, 240) # 悬停边框
MENU_CHECK = (30, 100, 220)         # 选中勾号颜色
SCROLL_BAR_BG = (235, 235, 235)     # 滚动条背景
SCROLL_THUMB = (190, 190, 190)      # 滑块颜色
SCROLL_THUMB_HOVER = (160, 160, 160) # 滑块悬停
...
# 存储下拉菜单滚动条相关几何信息（用于拖拽）
scroll_info = {}   # 格式: {"res": {"bar_rect":..., "total":..., "visible":...}, "disp": ...}
...
def draw_dropdown_menu(option_rect, items, current_index, menu_type):
    """绘制带滚动条的下拉菜单，支持滚轮和拖拽。返回可见项的矩形列表用于点击检测。"""
    global dropdown_rects, resolution_scroll, display_scroll, scrolling, scroll_target, scroll_info
    ...
    if total_items <= max_visible:
        ...
        if menu_type == "res":
            scroll_offset = max(0, min(resolution_scroll, total_items - max_visible))
        else:
            scroll_offset = max(0, min(display_scroll, total_items - max_visible))
    ...
    if total_items > max_visible:
        ...
        # 存储滚动条信息供拖拽使用
        scroll_info[menu_type] = {
            'bar_rect': pygame.Rect(scroll_bar_x, scroll_bar_y, 8, scroll_bar_height),
        ...
        # 若菜单不需要滚动条，清除对应信息
        if menu_type in scroll_info:
            del scroll_info[menu_type]
...
def get_enemy_new_direction(enemy, foods_list):
    ...
    if game_mode == "timed":
        ...
        if enemy_score < 20:
            ...
            if enemy['state'] == 'idle':
                ...
                if tactic == 0:
                    ...
                    perp_x = -player_dir[1]; perp_y = player_dir[0]; offset = 200 * SCALE; ahead_dist = base_ahead_dist * 0.8
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    target_x = base_target[0] + perp_x * offset; target_y = base_target[1] + perp_y * offset
                elif tactic == 2:
                    perp_x = -player_dir[1]; perp_y = player_dir[0]; offset = -200 * SCALE; ahead_dist = base_ahead_dist * 0.8
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    target_x = base_target[0] + perp_x * offset; target_y = base_target[1] + perp_y * offset
                    ...
                    target_x = predicted_player[0] - player_dir[0] * behind_dist; target_y = predicted_player[1] - player_dir[1] * behind_dist
                elif tactic == 4:
                    away_dist = 250 * SCALE
                    target_x = player_head[0] - player_dir[0] * away_dist; target_y = player_head[1] - player_dir[1] * away_dist
                elif tactic == 5:
                    perp_x = -player_dir[1]; perp_y = player_dir[0]; t = (name_hash % 1000) / 500.0 - 1.0; offset = t * 250 * SCALE
                    ahead_dist = base_ahead_dist * 0.7
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    target_x = base_target[0] + perp_x * offset; target_y = base_target[1] + perp_y * offset
                elif tactic == 6:
                    spread = 250 * SCALE; angle = (name_hash % 100) / 100.0 * math.pi * 2
                    offset_x = math.cos(angle) * spread; offset_y = math.sin(angle) * spread; ahead_dist = base_ahead_dist
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    target_x = base_target[0] + offset_x; target_y = base_target[1] + offset_y
                elif tactic == 7:
                    wait_dist = 300 * SCALE
                    target_x = predicted_player[0] + player_dir[0] * wait_dist; target_y = predicted_player[1] + player_dir[1] * wait_dist
                elif tactic == 8:
                    ahead_dist = base_ahead_dist
                    base_target = (predicted_player[0] + player_dir[0] * ahead_dist, predicted_player[1] + player_dir[1] * ahead_dist)
                    jitter = (name_hash % 200) - 100; perp_x = -player_dir[1]; perp_y = player_dir[0]
                    target_x = base_target[0] + perp_x * jitter; target_y = base_target[1] + perp_y * jitter
                else:
                    ahead_dist = base_ahead_dist
                    target_x = predicted_player[0] + player_dir[0] * ahead_dist; target_y = predicted_player[1] + player_dir[1] * ahead_dist

                dx = target_x - head[0]; dy = target_y - head[1]; length = math.hypot(dx, dy)
                if length > 1e-6:
                    desired_dir = (dx / length, dy / length)
                    if will_collide_with_player(head, desired_dir, radius, check_dist=30):
                        evade = get_evade_direction(head, all_segments, old_dir, radius)
                        if evade is not None: return evade
                    if direction_will_collide(head, desired_dir, all_segments, radius):
                        best_dir = None; best_dot = -float('inf')
                        for angle_offset in [-1.2, -0.9, -0.6, -0.3, 0.3, 0.6, 0.9, 1.2]:
                            ...
                            if test_dir[0] == -old_dir[0] and test_dir[1] == -old_dir[1]: continue
                            if not direction_will_collide(head, test_dir, all_segments, radius):
                                dot = test_dir[0] * desired_dir[0] + test_dir[1] * desired_dir[1]
                                if dot > best_dot: best_dot = dot; best_dir = test_dir
                        if best_dir is not None: return best_dir
                    else:
                        if not (desired_dir[0] == -old_dir[0] and desired_dir[1] == -old_dir[1]): return desired_dir
        ...
        player_head = snake[0]; player_dir = direction; player_speed = get_current_speed(); predicted_player = predict_player_position()
        if MOJIN_PHASE == "search":
            if containers:
                min_dist = float('inf'); nearest_container = None
                for cx, cy, cr, _, _ in containers:
                    dist = (head[0] - cx) ** 2 + (head[1] - cy) ** 2
                    if dist < min_dist: min_dist = dist; nearest_container = (cx, cy)
                if nearest_container: target_x, target_y = nearest_container
                else:
                    angle = random.uniform(0, 2*math.pi); r = random.uniform(100, 300)*SCALE
                    target_x = player_head[0] + math.cos(angle)*r; target_y = player_head[1] + math.sin(angle)*r
            else:
                angle = random.uniform(0, 2*math.pi); r = random.uniform(100, 300)*SCALE
                target_x = player_head[0] + math.cos(angle)*r; target_y = player_head[1] + math.sin(angle)*r
            ...
            dx = head[0]-player_head[0]; dy = head[1]-player_head[1]; length = math.hypot(dx,dy)
            if length>0: target_x = head[0] + (dx/length)*500*SCALE; target_y = head[1] + (dy/length)*500*SCALE
            else: target_x = head[0]+random.choice([-500,500])*SCALE; target_y = head[1]+random.choice([-500,500])*SCALE
            target_x = max(WORLD_MIN_X+50*SCALE, min(WORLD_MAX_X-50*SCALE, target_x))
            target_y = max(WORLD_MIN_Y+50*SCALE, min(WORLD_MAX_Y-50*SCALE, target_y))
        dx = target_x-head[0]; dy = target_y-head[1]; length = math.hypot(dx,dy)
        if length>1e-6:
        ...
        if enemy_score < 15:
            repulsion = [0.0,0.0]
            for sx,sy in all_segments:
                dx=head[0]-sx; dy=head[1]-sy; dist_sq=dx*dx+dy*dy
                if dist_sq < avoid_threshold_sq and dist_sq>0:
                    force=1.0/(dist_sq+1e-6); length=math.sqrt(dist_sq)
                    repulsion[0] += (dx/length)*force; repulsion[1] += (dy/length)*force
            rep_length = math.hypot(*repulsion)
            if rep_length>1e-6:
                new_dir = (repulsion[0]/rep_length, repulsion[1]/rep_length)
                if not (new_dir[0]==-old_dir[0] and new_dir[1]==-old_dir[1]): return new_dir
        else:
            target_enemy=None; min_dist_sq=float('inf')
            for other in enemies:
                if other is enemy: continue
                other_head = other['body'][0]; dx=other_head[0]-head[0]; dy=other_head[1]-head[1]; dist_sq=dx*dx+dy*dy
                if dist_sq<min_dist_sq: min_dist_sq=dist_sq; target_enemy=other
            if target_enemy:
                target_head=target_enemy['body'][0]; target_dir=target_enemy['dir']; ahead_dist=50*SCALE
                target_ahead=(target_head[0]+target_dir[0]*ahead_dist, target_head[1]+target_dir[1]*ahead_dist)
                dx=target_ahead[0]-head[0]; dy=target_ahead[1]-head[1]; length=math.hypot(dx,dy)
                if length>1e-6:
                    new_dir=(dx/length, dy/length)
                    if not (new_dir[0]==-old_dir[0] and new_dir[1]==-old_dir[1]): return new_dir
                dx=target_head[0]-head[0]; dy=target_head[1]-head[1]; length=math.hypot(dx,dy)
                if length>1e-6:
                    new_dir=(dx/length, dy/length)
                    if not (new_dir[0]==-old_dir[0] and new_dir[1]==-old_dir[1]): return new_dir

    if foods_list and random.random() < ENEMY_FOOD_SEEK_PROB:
        min_dist_sq=float('inf'); target_food=None
        for fx,fy,_,_ in foods_list:
            d_sq=(head[0]-fx)**2+(head[1]-fy)**2
            if d_sq<min_dist_sq: min_dist_sq=d_sq; target_food=(fx,fy)
        if target_food:
            dx=target_food[0]-head[0]; dy=target_food[1]-head[1]; length=math.hypot(dx,dy)
            if length>1e-6:
                new_dir=(dx/length, dy/length)
                if not (new_dir[0]==-old_dir[0] and new_dir[1]==-old_dir[1]): return new_dir

    options=[(1,0),(-1,0),(0,1),(0,-1)]; opposite=(-old_dir[0],-old_dir[1])
    options=[d for d in options if d!=opposite]
    if options: return random.choice(options)
    return old_dir

def check_boundary(head):
    x,y=head; margin=get_segment_radius()
    return (x<WORLD_MIN_X+margin or x>WORLD_MAX_X-margin or y<WORLD_MIN_Y+margin or y>WORLD_MAX_Y-margin)
...
def generate_obstacles_and_containers():
    ...
    for _ in range(OBSTACLE_COUNT):
        placed=False
        for _ in range(200):
            x=random.randint(WORLD_MIN_X+margin, WORLD_MAX_X-margin); y=random.randint(WORLD_MIN_Y+margin, WORLD_MAX_Y-margin)
            overlap=False
            for sx,sy in snake:
                if (x-sx)**2+(y-sy)**2 < (OBSTACLE_RADIUS+get_segment_radius())**2: overlap=True; break
            if overlap: continue
            for enemy in enemies:
                for sx,sy in enemy['body']:
                    if (x-sx)**2+(y-sy)**2 < (OBSTACLE_RADIUS+get_segment_radius())**2: overlap=True; break
                if overlap: break
            if overlap: continue
            if is_overlap(x,y,OBSTACLE_RADIUS,obstacles): continue
            if is_overlap(x,y,OBSTACLE_RADIUS,containers): continue
    ...
    for _ in range(CONTAINER_COUNT):
        ...
        for _ in range(200):
            ...
            overlap=False
            for sx,sy in snake:
                if (x-sx)**2+(y-sy)**2 < (CONTAINER_RADIUS+get_segment_radius())**2: overlap=True; break
            if overlap: continue
            for enemy in enemies:
                for sx,sy in enemy['body']:
                    if (x-sx)**2+(y-sy)**2 < (CONTAINER_RADIUS+get_segment_radius())**2: overlap=True; break
                if overlap: break
            if overlap: continue
            if is_overlap(x,y,CONTAINER_RADIUS,obstacles): continue
            if is_overlap(x,y,CONTAINER_RADIUS,containers): continue
            ...
            containers.append((x,y,CONTAINER_RADIUS,reward_type,reward_value)); placed=True; break
        if not placed:
            x=random.randint(WORLD_MIN_X+margin, WORLD_MAX_X-margin); y=random.randint(WORLD_MIN_Y+margin, WORLD_MAX_Y-margin)
...
def handle_container_collision(segment):
    ...
    for i,(cx,cy,cr,reward_type,reward_val) in enumerate(containers):
        if (head[0]-cx)**2+(head[1]-cy)**2 < (cr+get_segment_radius())**2:
            del containers[i]
            if reward_type==REWARD_SCORE: score+=reward_val
            ...
            elif reward_type==REWARD_BOOST: temp_boost_remaining=reward_val
            return True
    return False

def draw_circle(x,y,color,radius):
    sx,sy=world_to_screen(x,y); pygame.draw.circle(screen,color,(sx,sy),radius)
...
def draw_background():
    ...
    if bg_value==BLACK: screen.fill(BLACK)
        ...
        screen.fill(WHITE)
        left_world=cam_x-SCREEN_WIDTH//2; right_world=cam_x+SCREEN_WIDTH//2; bottom_world=cam_y-SCREEN_HEIGHT//2; top_world=cam_y+SCREEN_HEIGHT//2
        start_x=math.floor(left_world/GRID_SIZE)*GRID_SIZE; end_x=math.ceil(right_world/GRID_SIZE)*GRID_SIZE
        start_y=math.floor(bottom_world/GRID_SIZE)*GRID_SIZE; end_y=math.ceil(top_world/GRID_SIZE)*GRID_SIZE
        x=start_x
        while x<end_x:
            y=start_y
            while y<end_y:
                color=WHITE if ((x//GRID_SIZE)+(y//GRID_SIZE))%2==0 else LIGHT_GRAY_BG
                sx,sy=world_to_screen(x,y); pygame.draw.rect(screen,color,(sx,sy,GRID_SIZE,GRID_SIZE))
                y+=GRID_SIZE
            x+=GRID_SIZE
    elif bg_value=="stars":
        screen.fill(BLACK)
        for _ in range(100):
            sx=random.randint(0,SCREEN_WIDTH); sy=random.randint(0,SCREEN_HEIGHT); brightness=random.randint(100,255)
...
def add_score(name,score_value):
    scores=load_scores(); scores.append([name,score_value]); scores.sort(key=lambda x:x[1],reverse=True); save_scores(scores)

def get_current_rank():
    all_scores=[score]+[e['score'] for e in enemies]; all_scores.sort(reverse=True)
    for i,s in enumerate(all_scores):
        if s==score: return i+1
    return 1

def get_sorted_rankings():
    rankings=[(player_name,score)]+[(e['name'],e['score']) for e in enemies]; rankings.sort(key=lambda x:x[1],reverse=True); return rankings

# ---- 各种界面函数 ----
def show_rankings():
    font_title=get_font(48); font_entry=get_font(24); font_button=get_font(24)
    title=font_title.render("淘汰之王历史排行榜",True,BLACK); back_button_text="返回 (ESC)"
    ...
    while waiting:
        ...
        screen.fill(START_BG); screen.blit(title,(SCREEN_WIDTH//2-title.get_width()//2,100*SCALE))
        y=int(200*SCALE)
        for i,(name,score_val) in enumerate(top10):
            line=f"{i+1}. {name} ({score_val})"; rendered=font_entry.render(line,True,BLACK)
            screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,y)); y+=int(35*SCALE)
        pygame.draw.rect(screen,LIGHT_GRAY,button_rect); pygame.draw.rect(screen,GRAY,button_rect,2)
...
def show_name_input_screen():
    font_title=get_font(48); font_prompt=get_font(24); font_input=get_font(24); font_button=get_font(24)
    title=font_title.render("欢迎来到自由贪吃蛇",True,BLACK); prompt=font_prompt.render("请输入你的名字 (最多20字符):",True,BLACK)
    button_text="确认 (Enter)"; input_text=""; waiting=True
    ...
    while waiting:
        ...
        screen.fill(START_BG); screen.blit(title,(SCREEN_WIDTH//2-title.get_width()//2,int(150*SCALE)))
        ...
        pygame.draw.rect(screen,WHITE,input_box_rect); pygame.draw.rect(screen,BLACK,input_box_rect,2)
        text_surface=font_input.render(input_text,True,BLACK); screen.blit(text_surface,(input_box_rect.x+10,input_box_rect.y+10))
        if pygame.time.get_ticks()%1000<500:
            cursor_x=input_box_rect.x+10+text_surface.get_width()
            pygame.draw.line(screen,BLACK,(cursor_x,input_box_rect.y+5),(cursor_x,input_box_rect.y+input_box_rect.height-5),2)
        pygame.draw.rect(screen,LIGHT_GRAY,button_rect); pygame.draw.rect(screen,GRAY,button_rect,2)
        btn_surface=font_button.render(button_text,True,BLACK)
        screen.blit(btn_surface,(button_rect.centerx-btn_surface.get_width()//2,button_rect.centery-btn_surface.get_height()//2))
        pygame.display.flip(); clock.tick(FPS)

def show_full_changelog():
    font_title=get_font(48); font_version=get_font(30); font_content=get_font(20); font_button=get_font(24)
    title=font_title.render("更新历史",True,BLACK); back_button_text="返回 (ESC)"
    button_rect=pygame.Rect(SCREEN_WIDTH//2-BUTTON_HALF_WIDTH,SCREEN_HEIGHT-100,BUTTON_WIDTH,BUTTON_HEIGHT)
    content_lines=[]
    ...
    total_lines=len(content_lines); max_visible_lines=20; scroll_offset=0; waiting=True
    while waiting:
        for event in pygame.event.get():
            if event.type==pygame.QUIT: pygame.quit(); exit()
            elif event.type==pygame.KEYDOWN:
                if event.key==pygame.K_ESCAPE: waiting=False
                elif event.key==pygame.K_UP: scroll_offset=max(0,scroll_offset-1)
                elif event.key==pygame.K_DOWN: scroll_offset=min(total_lines-max_visible_lines,scroll_offset+1)
            elif event.type==pygame.MOUSEBUTTONDOWN:
                if event.button==1 and button_rect.collidepoint(event.pos): waiting=False
                elif event.button==4: scroll_offset=max(0,scroll_offset-3)
                elif event.button==5: scroll_offset=min(total_lines-max_visible_lines,scroll_offset+3)
        screen.fill(START_BG); screen.blit(title,(SCREEN_WIDTH//2-title.get_width()//2,int(50*SCALE)))
        y=int(150*SCALE)
        for i in range(scroll_offset,min(scroll_offset+max_visible_lines,total_lines)):
            ...
            if line_type=="version":
                rendered=font_version.render(text,True,BLACK); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,y)); y+=int(30*SCALE)
            elif line_type=="content":
                rendered=font_content.render("  • "+text,True,BLACK); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,y)); y+=int(25*SCALE)
            else: y+=int(10*SCALE)
        pygame.draw.rect(screen,LIGHT_GRAY,button_rect); pygame.draw.rect(screen,GRAY,button_rect,2)
        btn_text=font_button.render(back_button_text,True,BLACK)
        screen.blit(btn_text,(button_rect.centerx-btn_text.get_width()//2,button_rect.centery-btn_text.get_height()//2))
...
def show_credits():
    font_title=get_font(48); font_credit=get_font(24); font_button=get_font(24)
    title=font_title.render("开发者名单",True,BLACK); back_button_text="返回 (ESC)"
    button_rect=pygame.Rect(SCREEN_WIDTH//2-BUTTON_HALF_WIDTH,SCREEN_HEIGHT-100,BUTTON_WIDTH,BUTTON_HEIGHT)
    waiting=True
    while waiting:
        for event in pygame.event.get():
        ...
        for line in CREDITS:
            if line:
                rendered=font_credit.render(line,True,BLACK); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,y)); y+=int(40*SCALE)
            else: y+=int(10*SCALE)
        pygame.draw.rect(screen,LIGHT_GRAY,button_rect); pygame.draw.rect(screen,GRAY,button_rect,2)
        btn_text=font_button.render(back_button_text,True,BLACK)
        screen.blit(btn_text,(button_rect.centerx-btn_text.get_width()//2,button_rect.centery-btn_text.get_height()//2))
...
def show_changelog():
    log_surface=pygame.Surface((SCREEN_WIDTH,SCREEN_HEIGHT),pygame.SRCALPHA); log_surface.fill((192,255,240,200))
    font_title=get_font(48); font_content=get_font(20); font_button=get_font(24)
    title=font_title.render("版本更新",True,BLACK); button_text="开始游戏"
    ...
    while waiting:
        ...
        screen.fill(BLACK); screen.blit(log_surface,(0,0)); screen.blit(title,(SCREEN_WIDTH//2-title.get_width()//2,int(200*SCALE)))
        y=int(300*SCALE)
        for line in CURRENT_CHANGELOG:
            rendered=font_content.render(line,True,BLACK); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,y)); y+=int(30*SCALE)
        pygame.draw.rect(screen,LIGHT_GRAY,button_rect); pygame.draw.rect(screen,GRAY,button_rect,2)
...
def draw():
    ...
    if graphics_settings_mode:
        ...
        font_big=get_font(36); font_opt=get_font(24); font_small=get_font(16)
    ...
    if settings_mode:
        ...
        for i,text in enumerate(items):
            ...
            font=get_font(24); rendered=font.render(display_text,True,BLACK)
            rect=rendered.get_rect(center=(SCREEN_WIDTH//2,y)); setting_option_rects.append(rect)
            screen.blit(rendered,rect.topleft); y+=int(60*SCALE)
    ...
    if not game_started:
        ...
        if game_mode=="timed": mode_text="模式: 淘汰之王  (M键切换)"; mode_color=YELLOW
        elif game_mode=="mojin": mode_text="模式: 搜打撤  (M键切换)"; mode_color=ORANGE
        else: mode_text="模式: 经典  (M键切换)"; mode_color=BLACK
        ...
        if mode_hover: mode_rendered=mode_font.render(mode_text,True,GRAY)
        ...
        for i,(y_btn,text) in enumerate(zip(BUTTON_Y_POS,button_texts)):
            screen_y=SCREEN_HEIGHT//2+y_btn; text_color=GRAY if hover_button==i else BLACK
        ...
        font_small=get_font(18); hint_text="按 H 查看历史排行榜"
    ...
    for cx,cy,cr,reward_type,_ in containers:
        color=(255,215,0) if reward_type==REWARD_SCORE else (0,255,0) if reward_type==REWARD_FOOD else (0,255,255)
        draw_circle(cx,cy,color,cr)
    head_color=HEAD_COLOR_OPTIONS[head_color_index]; body_color=BODY_COLOR_OPTIONS[body_color_index]
    current_radius=get_segment_radius()
    for i,(x,y) in enumerate(snake): draw_circle(x,y,head_color if i==0 else body_color,current_radius)
    for enemy in enemies:
        for i,(x,y) in enumerate(enemy['body']): draw_circle(x,y,RED if i==0 else DARK_RED,current_radius)
    for fx,fy,fcolor,fr in foods: draw_circle(fx,fy,fcolor,fr)
    ...
    bg_value=BACKGROUND_STYLE_VALUES[background_style]; text_color=WHITE if bg_value==BLACK else BLACK
    font_info=get_font(16); font_small=get_font(12)
    screen.blit(font_info.render(f"Score: {score}",True,text_color),(SCREEN_WIDTH-int(200*SCALE),int(30*SCALE)))
    speed_val=get_current_speed(); speed_text=f"Speed: {speed_val:.1f}"
    if boosting and (time.time()-boost_start_time)<BOOST_DURATION:
        speed_text+=f" (Boost: {BOOST_DURATION-(time.time()-boost_start_time):.1f}s)"
    if temp_boost_remaining>0: speed_text+=f" (Temp Boost: {temp_boost_remaining//60}s)"
    ...
    if game_mode=="timed":
        ...
        screen.blit(font_small.render(f"狂暴敌人: {high_score_count}/{attack_trigger_count}",True,rage_color),(int(20*SCALE),int(90*SCALE)))
    if game_mode in ("timed","mojin"):
        rankings=get_sorted_rankings(); rank_font=get_font(14)
        rank_surface=pygame.Surface((int(200*SCALE),int(350*SCALE)),pygame.SRCALPHA); rank_surface.fill((0,0,0,100))
        screen.blit(rank_surface,(SCREEN_WIDTH-int(220*SCALE),int(120*SCALE)))
        screen.blit(rank_font.render("实时排名",True,WHITE),(SCREEN_WIDTH-int(220*SCALE)+10,int(130*SCALE)))
        ...
        for i,(name,scr) in enumerate(rankings[:15]):
            ...
            screen.blit(rank_font.render(line,True,color),(SCREEN_WIDTH-int(220*SCALE)+10,y_offset)); y_offset+=int(20*SCALE)
        if len(rankings)>15:
            screen.blit(rank_font.render(f"... 共 {len(rankings)} 名",True,WHITE),(SCREEN_WIDTH-int(220*SCALE)+10,y_offset))
    if game_mode=="mojin":
        phase_font=get_font(20); phase_colors={"search":CYAN,"strike":RED,"withdraw":ORANGE}
        phase_names={"search":"搜索","strike":"打击","withdraw":"撤离"}
        screen.blit(phase_font.render(f"阶段: {phase_names[MOJIN_PHASE]}",True,phase_colors[MOJIN_PHASE]),(int(20*SCALE),int(120*SCALE)))
    if show_fps:
        screen.blit(font_small.render(f"FPS: {int(clock.get_fps())}",True,text_color),(SCREEN_WIDTH-int(100*SCALE),SCREEN_HEIGHT-int(30*SCALE)))
    ...
    if not game_active:
        overlay=pygame.Surface((SCREEN_WIDTH,SCREEN_HEIGHT),pygame.SRCALPHA); overlay.fill((128,128,128,200)); screen.blit(overlay,(0,0))
        go_font=get_font(48); screen.blit(go_font.render("游戏结束",True,WHITE),(SCREEN_WIDTH//2-go_font.size("游戏结束")[0]//2,int(200*SCALE)))
        if game_mode=="timed" and gameover_killer_name:
            current_rank=get_current_rank(); text=f"被 {gameover_killer_name} 淘汰，本局排名第 {current_rank}"
            killer_font=get_font(24); screen.blit(killer_font.render(text,True,WHITE),(SCREEN_WIDTH//2-killer_font.size(text)[0]//2,int(270*SCALE)))
        score_font=get_font(32); screen.blit(score_font.render(f"最终得分: {score}",True,WHITE),(SCREEN_WIDTH//2-score_font.size(f"最终得分: {score}")[0]//2,int(320*SCALE)))
        option_font=get_font(36); option_y_start=int(400*SCALE)
        for i,opt in enumerate(GAMEOVER_OPTIONS):
            text="▶ "+opt+" ◀" if i==gameover_selection else opt; color=YELLOW if i==gameover_selection else WHITE
            rendered=option_font.render(text,True,color); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,option_y_start+i*int(60*SCALE)))
    ...
    if paused:
        ...
        pause_font=get_font(48); screen.blit(pause_font.render("暂停",True,WHITE),(SCREEN_WIDTH//2-pause_font.size("暂停")[0]//2,int(200*SCALE)))
        option_font=get_font(36); option_y_start=int(350*SCALE)
        for i,opt in enumerate(PAUSE_OPTIONS):
            text="▶ "+opt+" ◀" if i==pause_selection else opt; color=YELLOW if i==pause_selection else WHITE
            rendered=option_font.render(text,True,color); screen.blit(rendered,(SCREEN_WIDTH//2-rendered.get_width()//2,option_y_start+i*int(60*SCALE)))
...
def restart_game():
    ...
    snake=[(0,0),(-16*SCALE,0),(-32*SCALE,0),(-48*SCALE,0),(-64*SCALE,0)]; direction=(1,0); score=0; game_active=True; boosting=False
    color_centers.clear(); enemies=[]; gameover_killer_name=None; global_attack_phase=False; MOJIN_PHASE="search"; MOJIN_TIMER=0; temp_boost_remaining=0
    ...
    for _ in range(enemy_count):
        ...
        for _ in range(500):
            ...
            overlap=False
            for sx,sy in snake:
                if (x-sx)**2+(y-sy)**2<min_dist_sq: overlap=True; break
            if overlap: continue
            for enemy in enemies:
                for sx,sy in enemy['body']:
                    if (x-sx)**2+(y-sy)**2<min_dist_sq: overlap=True; break
                if overlap: break
            if not overlap:
                body=[(x,y),(x-16*SCALE,y),(x-32*SCALE,y)]; dir=random.choice([(1,0),(-1,0),(0,1),(0,-1)])
                name=random.choice(ENEMY_NAMES) if game_mode=="timed" else "Enemy"
                enemies.append({'body':body,'dir':dir,'score':0,'name':name,'state':'idle'}); placed=True; break
        if not placed:
            x=random.randint(WORLD_MIN_X+int(100*SCALE),WORLD_MAX_X-int(100*SCALE))
            y=random.randint(WORLD_MIN_Y+int(100*SCALE),WORLD_MAX_Y-int(100*SCALE))
            body=[(x,y),(x-16*SCALE,y),(x-32*SCALE,y)]; dir=random.choice([(1,0),(-1,0),(0,1),(0,-1)])
...
def settings_cancel():
    ...
    background_style=DEFAULT_BACKGROUND_STYLE; head_color_index=HEAD_COLOR_OPTIONS.index(DEFAULT_HEAD_COLOR)
    body_color_index=BODY_COLOR_OPTIONS.index(DEFAULT_BODY_COLOR); settings_mode=False
...
new_game(); last_time=time.time(); running=True; gameover_killer_name=None; global_attack_phase=False
MOJIN_PHASE="search"; MOJIN_TIMER=0; temp_boost_remaining=0
...
while running:
        ...
        if event.type==pygame.QUIT: running=False
            ...
            if scrolling and scroll_target and scroll_target in scroll_info:
                ...
                if bar.height > thumb_h:
                    ...
                    new_offset = int(ratio * (total - visible) + 0.5)
                    new_offset = max(0, min(new_offset, total - visible))
                    if scroll_target == "res":
                        resolution_scroll = new_offset
                    elif scroll_target == "disp":
                        display_scroll = new_offset
            if graphics_settings_mode:
                ...
                for i in range(3):
                    font=get_font(36); rendered=font.render(PAUSE_OPTIONS[i],True,WHITE)
                    rect=pygame.Rect(SCREEN_WIDTH//2-rendered.get_width()//2-10,
                                     option_y_start+i*int(60*SCALE)-10,rendered.get_width()+20,rendered.get_height()+20)
                    if rect.collidepoint(mouse_x,mouse_y): pause_selection=i; break
            elif gameover_screen:
                option_y_start=int(400*SCALE)
                for i in range(2):
                    font=get_font(36); rendered=font.render(GAMEOVER_OPTIONS[i],True,WHITE)
                    rect=pygame.Rect(SCREEN_WIDTH//2-rendered.get_width()//2-10,
                                     option_y_start+i*int(60*SCALE)-10,rendered.get_width()+20,rendered.get_height()+20)
            ...
            if graphics_settings_mode:
                if event.key==pygame.K_ESCAPE: graphics_settings_mode=False; resolution_dropdown=display_dropdown=False
                elif event.key==pygame.K_UP: graphics_selected=(graphics_selected-1)%2; resolution_dropdown=display_dropdown=False
                elif event.key==pygame.K_DOWN: graphics_selected=(graphics_selected+1)%2; resolution_dropdown=display_dropdown=False
                    ...
                    if graphics_selected==0: resolution_dropdown=not resolution_dropdown; display_dropdown=False
                    else: display_dropdown=not display_dropdown; resolution_dropdown=False
                continue
            elif settings_mode:
            ...
            if graphics_settings_mode:
                back_font=get_font(24); back_text=back_font.render("返回",True,BLACK)
                back_rect=back_text.get_rect(topleft=(BACK_BUTTON_POS[0]+20,BACK_BUTTON_POS[1]+20))
                if back_rect.collidepoint(mouse_x,mouse_y): graphics_settings_mode=False; resolution_dropdown=display_dropdown=False; continue
                ...
                if res_rect.collidepoint(mouse_x,mouse_y): graphics_selected=0; resolution_dropdown=not resolution_dropdown; display_dropdown=False
                elif disp_rect.collidepoint(mouse_x,mouse_y): graphics_selected=1; display_dropdown=not display_dropdown; resolution_dropdown=False
                else:
                    # 点击下拉菜单项
                    for dtype,idx,rect in dropdown_rects:
                        if rect.collidepoint(mouse_x,mouse_y):
                            if dtype=="res": current_resolution_index=idx
                            else: current_display_mode_index=idx
                            apply_display_settings(); resolution_dropdown=display_dropdown=False; break
                    # 如果点击了滚动条滑块，开始拖拽
                    if not resolution_dropdown and not display_dropdown:
                        for menu_type, info in scroll_info.items():
                            if info['thumb_rect'].collidepoint(mouse_x,mouse_y):
                                scrolling = True
                                scroll_target = menu_type
                                break
                continue
            elif settings_mode:
                back_font=get_font(24); back_text=back_font.render("返回",True,BLACK)
            ...
            if graphics_settings_mode:
                if resolution_dropdown:
                    resolution_scroll=max(0,min(resolution_scroll-event.y,len(RESOLUTIONS)-VISIBLE_ITEMS))
                if display_dropdown:
                    display_scroll=max(0,min(display_scroll-event.y,len(DISPLAY_MODES)-VISIBLE_ITEMS))
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        wx,wy=screen_to_world(mouse_x,mouse_y); hx,hy=snake[0]; dx=wx-hx; dy=wy-hy
        if abs(dx)>1 or abs(dy)>1: length=math.hypot(dx,dy); direction=(dx/length,dy/length)
        current_speed=get_current_speed(); current_radius=get_segment_radius()
        eat_threshold_sq=(current_radius+FOOD_RADIUS)**2; large_eat_threshold_sq=(current_radius+LARGE_FOOD_RADIUS)**2
        collision_threshold_sq=(current_radius*2)**2
        old_head=snake[0]; new_head=(old_head[0]+direction[0]*current_speed,old_head[1]+direction[1]*current_speed)
        handle_container_collision(new_head)
        new_enemy_heads=[]
        for enemy in enemies:
            old_enemy_head=enemy['body'][0]; enemy_speed=get_base_speed()
            if game_mode=="timed" and enemy['score']>=20 and enemy.get('state')=='attacking': enemy_speed*=4.0
            elif game_mode=="mojin":
                if MOJIN_PHASE=="strike": enemy_speed*=MOJIN_STRIKE_SPEED
                elif MOJIN_PHASE=="withdraw": enemy_speed*=MOJIN_WITHDRAW_SPEED
                else: enemy_speed*=MOJIN_SEARCH_SPEED
            new_dir=get_enemy_new_direction(enemy,foods); enemy['dir']=new_dir
            new_enemy_head=(old_enemy_head[0]+new_dir[0]*enemy_speed,old_enemy_head[1]+new_dir[1]*enemy_speed)
        ...
        for enemy,old_head,new_head_pos in new_enemy_heads:
            if check_boundary(new_head_pos): spawn_foods_from_enemy(enemy['body']); enemies.remove(enemy)
            else: surviving_enemy_heads.append((enemy,old_head,new_head_pos)); handle_container_collision(new_head_pos)
        ate_index=None
        for i,(fx,fy,_,fr) in enumerate(foods):
            threshold=large_eat_threshold_sq if fr==LARGE_FOOD_RADIUS else eat_threshold_sq
            if (new_head[0]-fx)**2+(new_head[1]-fy)**2<threshold: ate_index=i; break
            mid_x=(old_head[0]+new_head[0])/2; mid_y=(old_head[1]+new_head[1])/2
            if (mid_x-fx)**2+(mid_y-fy)**2<threshold: ate_index=i; break
        if ate_index is not None: score+=1
        enemy_ate_info=[]
        for enemy,old_enemy_head,new_enemy_head in surviving_enemy_heads:
            for i,(fx,fy,_,fr) in enumerate(foods):
                threshold=large_eat_threshold_sq if fr==LARGE_FOOD_RADIUS else eat_threshold_sq
                if (new_enemy_head[0]-fx)**2+(new_enemy_head[1]-fy)**2<threshold:
                    enemy_ate_info.append((enemy,i)); enemy['score']+=1; break
                mid_x=(old_enemy_head[0]+new_enemy_head[0])/2; mid_y=(old_enemy_head[1]+new_enemy_head[1])/2
                if (mid_x-fx)**2+(mid_y-fy)**2<threshold: enemy_ate_info.append((enemy,i)); enemy['score']+=1; break
        foods_to_remove=set()
        if ate_index is not None: foods_to_remove.add(ate_index)
        for enemy,i in enemy_ate_info: foods_to_remove.add(i)
        snake.insert(0,new_head)
        if ate_index is None: snake.pop()
        for enemy,old_head_pos,new_head_pos in surviving_enemy_heads:
            enemy['body'].insert(0,new_head_pos)
            if not any(enemy==e for e,_ in enemy_ate_info): enemy['body'].pop()
        for i in sorted(foods_to_remove,reverse=True): del foods[i]
        while len(foods)<FOOD_COUNT:
            new_color=random.choice(FOOD_COLORS); new_food=create_food(new_color,FOOD_RADIUS)
            if new_food: foods.append(new_food)
            else: pos=random_position(); foods.append((pos[0],pos[1],new_color,FOOD_RADIUS))
        player_head=snake[0]
        for enemy in enemies:
        ...
        for enemy in enemies:
            ...
            for segment in snake[1:]:
                if (enemy_head[0]-segment[0])**2+(enemy_head[1]-segment[1])**2<collision_threshold_sq:
                    enemies_to_remove.append(enemy); break
            ...
            for other in enemies:
                ...
                for segment in other['body'][1:]:
                    if (enemy_head[0]-segment[0])**2+(enemy_head[1]-segment[1])**2<collision_threshold_sq:
                        enemies_to_remove.append(enemy); break
                if enemy in enemies_to_remove: break
        ...
        if game_mode=="timed":
            ...
            if global_attack_phase:
                if high_score_count<attack_trigger_count*0.7: global_attack_phase=False; [e.update({'state':'idle'}) for e in enemies]
            else:
