...
base_red_hp = BASE_HP
base_blue_pos = (-1800*SCALE, 0)
base_red_pos = (1800*SCALE, 0)
respawn_timers = {}
...
def get_enemy_new_direction(enemy, all_enemies):
    ...
    if game_mode == "team4v4":
        if enemy_team == "blue":
            for e in enemy_team_data:
                hostile_segments.extend(e['body'])
        ...
        for other in all_enemies:
            if other is not enemy:
                hostile_segments.extend(other['body'])
...
def draw():
    ...
    if not game_active:
        ...
        if game_mode == "team4v4":
            ...
            screen.blit(gof.render(result_text, True, WHITE), (SCREEN_WIDTH//2 - 100, 200*SCALE))
        else:
        ...
        for i, opt in enumerate(GAMEOVER_OPTIONS):
            ...
            draw_option_rect(screen, rect, opt if i!=gameover_selection else "▶ "+opt+" ◀", get_font(36), is_selected=i==gameover_selection, base_color=WHITE)
    if paused:
        overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.SRCALPHA); overlay.fill((0,0,0,128))
        screen.blit(overlay, (0,0))
        pf = get_font(48); screen.blit(pf.render("暂停", True, WHITE), (SCREEN_WIDTH//2 - 60, 200*SCALE))
        oy = int(350*SCALE)
        for i, opt in enumerate(PAUSE_OPTIONS):
            rect = pygame.Rect(SCREEN_WIDTH//2 - 200, oy + i*int(60*SCALE) - 20, 400, 40)
            draw_option_rect(screen, rect, opt if i!=pause_selection else "▶ "+opt+" ◀", get_font(36), is_selected=i==pause_selection, base_color=WHITE)
    pygame.display.flip()
...
def restart_game():
    ...
    if game_mode == "team4v4":
        ...
        for i in range(3):
            x = base_blue_pos[0] + random.randint(-200*SCALE, 200*SCALE)
            y = base_blue_pos[1] + random.randint(-200*SCALE, 200*SCALE)
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
            teammates.append({'body': body, 'dir': (1,0), 'team': 'blue', 'name': f"队友{i+1}"})
        for i in range(4):
            x = base_red_pos[0] + random.randint(-200*SCALE, 200*SCALE)
            y = base_red_pos[1] + random.randint(-200*SCALE, 200*SCALE)
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
            enemy_team_data.append({'body': body, 'dir': (-1,0), 'team': 'red', 'name': f"敌方{i+1}"})
        generate_foods(FOOD_COUNT//2)
    else:
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT: running = False
                ...
                if game_mode == "team4v4" and game_active and not paused:
                ...
                if graphics_settings_mode:
                    ...
                    if res_rect.collidepoint(mouse_x, mouse_y): graphics_selected = 0; resolution_dropdown = not resolution_dropdown; display_dropdown = theme_dropdown = fps_dropdown = False
                    elif disp_rect.collidepoint(mouse_x, mouse_y): graphics_selected = 1; display_dropdown = not display_dropdown; resolution_dropdown = theme_dropdown = fps_dropdown = False
                    elif theme_rect.collidepoint(mouse_x, mouse_y): graphics_selected = 2; theme_dropdown = not theme_dropdown; resolution_dropdown = display_dropdown = fps_dropdown = False
                    elif fps_rect.collidepoint(mouse_x, mouse_y): graphics_selected = 3; fps_dropdown = not fps_dropdown; resolution_dropdown = display_dropdown = theme_dropdown = False
                    continue
                if settings_mode:
        ...
        if game_started and game_active and not paused and not gameover_screen:
            if game_mode == "team4v4":
                ...
                # 基地碰撞检测
                for tm in teammates:
                    if math.hypot(tm['body'][0][0]-base_red_pos[0], tm['body'][0][1]-base_red_pos[1]) < BASE_RADIUS + get_segment_radius():
                        base_red_hp -= 1
                for enemy in enemy_team_data:
                    if math.hypot(enemy['body'][0][0]-base_blue_pos[0], enemy['body'][0][1]-base_blue_pos[1]) < BASE_RADIUS + get_segment_radius():
                        base_blue_hp -= 1
                if math.hypot(snake[0][0]-base_red_pos[0], snake[0][1]-base_red_pos[1]) < BASE_RADIUS + get_segment_radius():
                    base_red_hp -= 1
