...
def get_font(size):
    ...
    print("警告：未找到中文字体，中文可能无法正常显示。请将 simhei.ttf 放在游戏目录下，或安装中文字体。")
...
def generate_obstacles_and_containers():
    ...
    for _ in range(OBSTACLE_COUNT):
        ...
        for attempt in range(100):
            ...
            if not overlap:
                ...
                for ox, oy, _ in obstacles:
                    if (x - ox) ** 2 + (y - oy) ** 2 < (OBSTACLE_RADIUS * 2) ** 2:
                        overlap = True
                        break
            if not overlap:
        ...
        if not placed:
            # 最后尝试放在角落
            obstacles.append((random.randint(WORLD_MIN_X + 100, WORLD_MAX_X - 100),
                              random.randint(WORLD_MIN_Y + 100, WORLD_MAX_Y - 100),
                              OBSTACLE_RADIUS))
    # 生成容器
    for _ in range(CONTAINER_COUNT):
        ...
        for attempt in range(100):
            ...
            if not overlap:
                for ox, oy, _ in obstacles:
                    if (x - ox) ** 2 + (y - oy) ** 2 < (CONTAINER_RADIUS + OBSTACLE_RADIUS) ** 2:
                        overlap = True
                        break
            if not overlap:
                for cx, cy, _, _, _ in containers:
                    if (x - cx) ** 2 + (y - cy) ** 2 < (CONTAINER_RADIUS * 2) ** 2:
                        overlap = True
                        break
            if not overlap:
                reward_type = random.choice([REWARD_SCORE, REWARD_FOOD, REWARD_BOOST])
                reward_value = 1 if reward_type == REWARD_SCORE else (random.randint(1,3) if reward_type == REWARD_FOOD else 120)  # 加速帧数
                containers.append((x, y, CONTAINER_RADIUS, reward_type, reward_value))
                placed = True
                break
        if not placed:
            containers.append((random.randint(WORLD_MIN_X + 100, WORLD_MAX_X - 100),
                               random.randint(WORLD_MIN_Y + 100, WORLD_MAX_Y - 100),
                               CONTAINER_RADIUS, REWARD_SCORE, 1))
