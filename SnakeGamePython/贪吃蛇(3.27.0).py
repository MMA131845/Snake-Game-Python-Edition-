...
# 全局攻击状态（仅淘汰之王模式）
global_attack_phase = False  # 是否进入协同攻击阶段
attack_trigger_count = 30    # 触发攻击所需的高分敌人数量
...
def will_collide_with_player(head, dir_vec, radius, check_dist=60):
    """检查是否会与玩家身体碰撞"""
    check_x = head[0] + dir_vec[0] * check_dist
    check_y = head[1] + dir_vec[1] * check_dist
    threshold_sq = (radius * 2) ** 2
...
def get_enemy_new_direction(enemy, foods_list):
    ...
    if game_mode == "timed":
        ...
        if not global_attack_phase and high_score_count >= attack_trigger_count:
            # 检查是否包围（简单条件：至少有一个敌人在玩家每个象限）
            # 这里简化：只要高分数量足够，就触发攻击
            global_attack_phase = True
            # 将所有高分敌人状态设为 'preparing'
            for e in enemies:
                if e['score'] >= 20:
                    e['state'] = 'preparing'
        ...
        if enemy_score < 20:
            ...
            if enemy['state'] == 'idle':
                ...
                if tactic == 0:  # 前锋：正面拦截
                    ...
                    target_x = base_target[0] + perp_x * offset
                    target_y = base_target[1] + perp_y * offset
                elif tactic == 6:  # 扇形包围：在玩家前方一定半径的弧线上
                    spread = 200
                ...
                elif tactic == 9:  # 假动作：先向一个方向移动，然后突然变向（这里暂时实现为直接冲向玩家前方，但可能被覆盖）
                    ahead_dist = base_ahead_dist
                    target_x = predicted_player[0] + player_dir[0] * ahead_dist
                    target_y = predicted_player[1] + player_dir[1] * ahead_dist
...
def restart_game():
    global snake, direction, foods, score, game_active, boosting, color_centers, enemies, gameover_killer_name, global_attack_phase
    snake = [(0, 0), (-16, 0), (-32, 0), (-48, 0), (-64, 0)]
    direction = (1, 0)
    score = 0
    game_active = True
...
while running:
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            # 计算敌人速度
            if game_mode == "timed" and enemy['score'] >= 20 and enemy['state'] == 'attacking':
        ...
        # 更新全局攻击阶段（如果高分敌人少于阈值，重置状态）
        if game_mode == "timed":
            high_score_count = sum(1 for e in enemies if e['score'] >= 20)
            if global_attack_phase:
                # 如果高分数量降得太低，退出攻击阶段
                if high_score_count < attack_trigger_count * 0.7:  # 低于70%时退出
                    global_attack_phase = False
                    for e in enemies:
                        e['state'] = 'idle'
            else:
                if high_score_count >= attack_trigger_count:
                    # 简单判定：一旦足够多高分，就进入准备阶段（实际可增加包围检测）
                    global_attack_phase = True
                    for e in enemies:
                        if e['score'] >= 20:
                            e['state'] = 'preparing'

            # 从准备阶段切换到攻击阶段（这里可以增加延迟或条件）
            if global_attack_phase:
                # 简单的延迟：如果准备超过一定时间或玩家靠近，触发攻击
                # 这里简化为当准备中的敌人数量多时，随机触发
                preparing_count = sum(1 for e in enemies if e['state'] == 'preparing')
                if preparing_count > attack_trigger_count * 0.8 and random.random() < 0.01:
                    for e in enemies:
