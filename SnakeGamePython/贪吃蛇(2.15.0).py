...
# ========== 游戏版本：v2.15.0 - 鼠标悬停效果 & ESC退出 ==========
VERSION = "v2.15.0"
CHANGELOG = f"""版本 {VERSION} - 鼠标悬停效果 & ESC退出

✨ 新增功能：
- 开始界面鼠标悬停在按钮上时显示浅灰色矩形框提示
...
def on_mouse_move(event):
    ...
    if not game_started and not settings_mode:
        button_width = 300
        button_height = 40
        button_positions = [50, 0, -50, -100]  # 与draw中的y坐标对应
        hover_button = None
        for i, y in enumerate(button_positions):
            if y - button_height/2 <= mouse_y <= y + button_height/2 and \
               -button_width/2 <= mouse_x <= button_width/2:
                hover_button = i
                break
    else:
        hover_button = None
...
def on_click(event):
    ...
    button_height = 40
    buttons = [
        {"func": start_game, "y": 50},
        {"func": enter_settings, "y": 0},
        {"func": restart, "y": -50},
        {"func": quit_game, "y": -100}
    ]
    for btn in buttons:
...
def draw():
    ...
    if settings_mode:
        ...
        for i, (y, text, font_info) in enumerate(zip(button_y, button_texts, button_fonts)):
            ...
            if hover_button == i:
                pen.penup()
                pen.goto(-150, y - 20)  # 矩形左下角
                pen.pendown()
                pen.color("#E0E0E0")    # 浅灰色
                pen.pensize(2)
                for _ in range(2):
                    pen.forward(300)
                    pen.left(90)
