...
try:
    ...
    print("警告：未找到 pywin32，IPC 通信将不可用。请安装：pip install pywin32")
    win32pipe = None
    win32file = None
...
def send_stats(fps, score, kills, mode="PVE"):
    """发送统计信息到启动器"""
    global pipe_client
...
# ========== 游戏版本 ==========
VERSION = "v3.0.0"
VERSION_FILE = os.path.join(os.path.expanduser("~"), ".snake_pve_version_pygame")
...
def check_version():
    ...
    if last_version != VERSION:
        print(f"版本更新提示：当前版本 {VERSION}，新功能：Pygame重写，流畅渲染。")
...
BUTTON_Y_POS = [45, -5, -55, -105]  # 相对于屏幕中心的Y坐标
...
def draw():
    ...
    if settings_mode:
        ...
        button_fonts = [get_font(24), get_font(18), get_font(18), get_font(18)]
        ...
        for i, (y, text) in enumerate(zip(BUTTON_Y_POS, button_texts)):
            ...
            screen.blit(rendered, (WIDTH//2 - rendered.get_width()//2, screen_y - rendered.get_height()//2 - 2))  # 文字偏移调整
        ...
        # 显示击杀数
        kills_text = font_small.render(f"击杀: {kills}", True, WHITE)
        screen.blit(kills_text, (20, 90))
        ...
        if not game_active:
            ...
            game_over = font_info.render("GAME OVER  按 R 重新开始", True, game_over_color)
...
while running:
    ...
    if game_started and not settings_mode and game_active:
        ...
        # ====== IPC 发送统计（每秒一次） ======
        current_clock_time = pygame.time.get_ticks() / 1000.0  # 秒
        if current_clock_time - last_ipc_send_time >= 1.0:
            fps = int(clock.get_fps())  # 实际帧率
            send_stats(fps, score, kills, "PVE")
            last_ipc_send_time = current_clock_time
