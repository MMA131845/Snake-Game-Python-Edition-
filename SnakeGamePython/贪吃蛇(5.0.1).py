...
def restart_game():
    ...
    if game_mode == "team4v4":
        ...
        for i in range(3):
            x = base_blue_pos[0] + random.randint(-int(200*SCALE), int(200*SCALE))
            y = base_blue_pos[1] + random.randint(-int(200*SCALE), int(200*SCALE))
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
            teammates.append({'body': body, 'dir': (1,0), 'team': 'blue', 'name': f"队友{i+1}"})
        for i in range(4):
            x = base_red_pos[0] + random.randint(-int(200*SCALE), int(200*SCALE))
            y = base_red_pos[1] + random.randint(-int(200*SCALE), int(200*SCALE))
            body = [(x, y), (x-16*SCALE, y), (x-32*SCALE, y)]
...
def main():
    ...
    # 下拉菜单相关全局变量（修复 UnboundLocalError）
