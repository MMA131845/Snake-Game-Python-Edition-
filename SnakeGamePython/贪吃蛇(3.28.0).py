...
# 实用功能标志
show_fps = True
performance_mode = False  # 性能模式（关闭抗锯齿和阴影）
...
def draw_circle(surface, x, y, color, radius):
    ...
    if performance_mode:
        ...
        pygame.draw.circle(aa_surface, color, (sx, sy), r)

def draw_shadow(surface, x, y, radius):
    """绘制阴影"""
    if performance_mode:
        sx, sy = world_to_screen(x, y)
        pygame.draw.circle(screen, (0,0,0), (sx+2, sy+2), radius)
    else:
        sx = int((x - cam_x + WIDTH // 2) / AA_SCALE) + 2 // AA_SCALE
        sy = int((y - cam_y + HEIGHT // 2) / AA_SCALE) + 2 // AA_SCALE
        r = int(radius / AA_SCALE)
        pygame.draw.circle(aa_surface, (0,0,0), (sx, sy), r)
...
def draw():
    ...
    # 如果非性能模式且不在游戏开始/设置界面，缩放抗锯齿表面
    if not performance_mode and not game_started and not settings_mode:
    ...
    # ========== 游戏进行中 ==========
        ...
        for i, (x, y) in enumerate(snake):
            ...
            draw_circle(aa_surface if not performance_mode else screen, x, y, color, current_radius)
        for enemy in enemies:
            for i, (x, y) in enumerate(enemy['body']):
                color = RED if i == 0 else DARK_RED
                draw_circle(aa_surface if not performance_mode else screen, x, y, color, current_radius)
        for fx, fy, fcolor, fr in foods:
            draw_circle(aa_surface if not performance_mode else screen, fx, fy, fcolor, fr)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            elif event.key == pygame.K_p:  # 按P切换性能模式
                if game_started and not paused and not gameover_screen:
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            if game_mode == "timed" and enemy['score'] >= 20 and enemy.get('state') == 'attacking':
                enemy_speed = get_base_speed() * 4.0  # 增强：速度提升至4倍
            elif game_mode == "mojin" and MOJIN_PHASE == "steal":
                enemy_speed = get_base_speed() * 2.5  # 摸金阶段加速
            else:
        ...
        if game_mode == "timed":
            ...
            if global_attack_phase:
                ...
                if preparing_count > attack_trigger_count * 0.8 and random.random() < 0.02:  # 触发概率提高
                    for e in enemies:
