...
def get_enemy_new_direction(enemy, foods_list):
    """
    根据游戏模式决定行为：
    - 淘汰之王模式：分数≥20时攻击玩家（绕前100像素，并加入协作偏移），分数<20时躲避所有蛇
    ...
    # 分数 < 阈值时总是躲避
    if (game_mode == "timed" and enemy_score < 20) or (game_mode == "classic" and enemy_score < 15):
        repulsion = [0.0, 0.0]
        ...
        if game_mode == "timed":  # 淘汰之王：攻击玩家（绕前100像素，协作偏移）
            player_head = snake[0]
            ...
            offset_magnitude = 50  # 基础偏移量
            if abs(dot) > 0.1:  # 如果AI不在玩家正前方很近的位置，根据叉积确定偏移方向
                if cross > 0:  # AI在玩家左侧
...
while running:
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            # 计算敌人速度：淘汰之王且分数≥20时加速3倍，否则为基础速度
