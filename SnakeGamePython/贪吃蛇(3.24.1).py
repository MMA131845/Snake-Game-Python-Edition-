...
def get_sorted_rankings():
    """获取本局所有实体（玩家+敌人）按分数降序排列的列表，每个元素为 (name, score)"""
...
def draw():
    global setting_option_rects, back_button_hover, mode_hover, gameover_killer_name, input_active, input_text
    screen.fill(BLACK)
    ...
    if settings_mode:
        ...
        # 玩家名称输入框
        name_prompt = font_small.render("请输入你的名字 (回车确认):", True, BLACK)
        screen.blit(name_prompt, (WIDTH//2 - name_prompt.get_width()//2, 250))

        # 输入框背景
        input_box_rect = pygame.Rect(WIDTH//2 - 150, 300, 300, 40)
        pygame.draw.rect(screen, WHITE, input_box_rect)
        pygame.draw.rect(screen, BLACK, input_box_rect, 2)

        # 当前输入文本
        display_name = input_text if input_active else player_name
        name_surface = font_input.render(display_name, True, BLACK)
        screen.blit(name_surface, (input_box_rect.x + 10, input_box_rect.y + 10))

        # 闪烁光标（当激活时）
        if input_active and pygame.time.get_ticks() % 1000 < 500:
            cursor_x = input_box_rect.x + 10 + name_surface.get_width()
...
def start_game():
    global game_started, settings_mode, player_name, input_text
    # 如果名字为空，使用默认名
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # 处理名字输入
            if not game_started and not settings_mode and input_active:
            ...
            if not game_started and not settings_mode:
                ...
                if input_box_rect.collidepoint(mouse_x, mouse_y):
                    ...
                    # 如果点击其他地方，可以取消激活，但为了易用性，保留激活状态直到回车
                    pass
            else:
                hover_button = None
                mode_hover = False
    ...
    if game_started and not settings_mode and game_active and not paused and not gameover_screen:
        ...
        for enemy in enemies:
            ...
            # 计算敌人速度：淘汰之王且分数≥20且高分总数>20时加速2倍，否则为基础速度
            if game_mode == "timed" and enemy['score'] >= 20:
                high_score_count = sum(1 for e in enemies if e['score'] >= 20)
                if high_score_count > 20:
                    enemy_speed = get_base_speed() * 2.0  # 改为2倍
                else:
        ...
        if check_boundary(new_head):
            ...
            if game_mode == "timed" and score > 0:
                add_score(player_name, score)  # 使用玩家名字
            draw()
            pygame.display.flip()
            continue
