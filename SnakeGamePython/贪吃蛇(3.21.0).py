...
def get_enemy_new_direction(enemy, foods_list):
    ...
    - 淘汰之王模式：只有当分数≥20的敌人数量 > 20时，分数≥20的敌人才会攻击玩家；
      攻击方式：战术角色分配（前锋/侧翼/后卫）、玩家轨迹预测、动态包围圈。
    ...
    if should_avoid:
        ...
        if game_mode == "timed":  # 淘汰之王：战术攻击
            ...
            if length > 1e-6:
                ...
                # 协同避免拥堵：检查新方向前方是否有其他AI，若有则微调方向
                perp_x = -new_dir[1]
                perp_y = new_dir[0]
                check_dist = 50
                check_point = (head[0] + new_dir[0] * check_dist,
                               head[1] + new_dir[1] * check_dist)
                for other in enemies:
                    if other is enemy:
                        continue
                    other_head = other['body'][0]
                    dist_to_other = math.hypot(check_point[0] - other_head[0],
                                               check_point[1] - other_head[1])
                    if dist_to_other < get_segment_radius() * 3:
                        # 尝试左右偏转
                        for sign in [1, -1]:
                            test_dir = (new_dir[0] + perp_x * sign * 0.3,
                                        new_dir[1] + perp_y * sign * 0.3)
                            test_len = math.hypot(test_dir[0], test_dir[1])
                            if test_len > 0:
                                test_dir = (test_dir[0] / test_len, test_dir[1] / test_len)
                            # 检查是否会掉头
                            if test_dir[0] == -old_dir[0] and test_dir[1] == -old_dir[1]:
                                continue
                            # 检查是否可行
                            test_point = (head[0] + test_dir[0] * check_dist,
                                          head[1] + test_dir[1] * check_dist)
                            dist_to_other = math.hypot(test_point[0] - other_head[0],
                                                       test_point[1] - other_head[1])
                            if dist_to_other >= get_segment_radius() * 3:
                                new_dir = test_dir
                                break
                        break
