...
def get_enemy_new_direction(enemy, foods_list):
    ...
    - 淘汰之王模式：只有当分数≥20的敌人数量 > 20时，分数≥20的敌人才会攻击玩家；
      攻击方式：预测玩家位置、动态绕前距离、智能分散包围、协同避免拥堵。
    ...
    if should_avoid:
        ...
        if game_mode == "timed":  # 淘汰之王：超级智能攻击
            player_head = snake[0]
            player_dir = direction
            player_speed = get_current_speed()

            # 预测玩家未来位置（基于当前速度和方向，预测0.5秒后）
            predict_time = 0.5  # 秒
            predicted_player = (player_head[0] + player_dir[0] * player_speed * predict_time,
                                 player_head[1] + player_dir[1] * player_speed * predict_time)

            # 动态绕前距离：基础100 + 玩家速度 * 10（速度越快，绕前越远）
            ...
            # 根据敌人名字的哈希值生成一个在 [-1, 1] 之间均匀分布的偏移系数
            # 这样可以使得不同名字的AI自然分散，形成包围
            name_hash = int(hashlib.md5(enemy['name'].encode()).hexdigest(), 16)
            t = (name_hash % 1000) / 500.0 - 1.0  # 范围 [-1, 1]
            spread_width = 150  # 包围宽度
            ...
            if length > 1e-6:
                ...
                if avoid_other_ai:
                    ...
                    for sign in [1, -1]:
                        ...
                        length = math.hypot(test_dir[0], test_dir[1])
                        if length > 0:
                            test_dir = (test_dir[0] / length, test_dir[1] / length)
                        # 检查是否会掉头
