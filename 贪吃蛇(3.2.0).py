...
def get_font(size):
    ...
    # 如果系统字体都不存在，尝试加载当前目录下的 simhei.ttf
    ...
    # 最后回退到默认字体（可能无法显示中文，但至少不崩溃）
    print("警告：未找到中文字体，中文可能无法正常显示。请安装中文字体或将 simhei.ttf 放在游戏目录下。")
    return pygame.font.Font(None, size)

# ========== 显示更新日志窗口 ==========
def show_changelog():
    """弹出一个半透明的窗口显示更新日志，点击后消失"""
    log_width, log_height = 600, 400
    log_surface = pygame.Surface((log_width, log_height), pygame.SRCALPHA)
    log_surface.fill((255, 255, 240, 220))  # 浅黄色半透明
    # 绘制边框
    pygame.draw.rect(log_surface, BLACK, log_surface.get_rect(), 2)
    # 绘制标题
    font_title = get_font(24)
    title = font_title.render("更新日志", True, BLACK)
    log_surface.blit(title, (log_width//2 - title.get_width()//2, 30))
    # 绘制内容
    font_content = get_font(18)
    y = 80
    for line in CHANGELOG:
        rendered = font_content.render(line, True, BLACK)
        log_surface.blit(rendered, (50, y))
        y += 30
    # 绘制关闭按钮
    font_button = get_font(20)
    btn_text = font_button.render("开始游戏", True, BLACK)
    btn_rect = pygame.Rect(log_width//2 - 80, log_height - 60, 160, 40)
    pygame.draw.rect(log_surface, LIGHT_GRAY, btn_rect)
    pygame.draw.rect(log_surface, GRAY, btn_rect, 2)
    log_surface.blit(btn_text, (btn_rect.centerx - btn_text.get_width()//2, btn_rect.centery - btn_text.get_height()//2))
    
    # 将窗口居中显示
    log_pos = (WIDTH//2 - log_width//2, HEIGHT//2 - log_height//2)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.button == 1:
                    mx, my = event.pos
                    # 检查是否点击了关闭按钮
                    if log_pos[0] + btn_rect.x <= mx <= log_pos[0] + btn_rect.x + btn_rect.width and \
                       log_pos[1] + btn_rect.y <= my <= log_pos[1] + btn_rect.y + btn_rect.height:
                        waiting = False
...
enemies = []  # 每个元素: {'body': [(x,y)...], 'dir': (dx,dy)}
foods = []    # 每个元素: (x, y, color, radius)
score = 0
game_active = True
game_started = False
settings_mode = False
...
color_centers = {}  # 用于食物聚集

mouse_x, mouse_y = 0, 0  # 屏幕坐标
mouse_pressed = False

pending_enemy_spawns = []  # 等待生成的敌人坐标
spawn_timer_active = False
last_spawn_check = 0

# 设置值
background_style = DEFAULT_BACKGROUND_STYLE
head_color_index = 0  # LIGHT_GREEN
body_color_index = 0  # GREEN
selected_setting = 0

# 悬停按钮
hover_button = None

# 食物生成函数（与turtle版类似，但用Pygame坐标）
...
def draw():
    ...
    if settings_mode:
        ...
        hint = font_small.render("↑↓选择  ←→更改  Enter保存  Esc取消", True, BLACK)
        ...
        if bg_value == "grid":
            ...
            while x <= end_x:
                ...
                pygame.draw.line(screen, color, (sx, 0), (sx, HEIGHT), 1)
                x += GRID_SIZE
            # 横线
            y = start_y
            while y <= end_y:
                _, sy = world_to_screen(0, y)
                pygame.draw.line(screen, color, (0, sy), (WIDTH, sy), 1)
                y += GRID_SIZE
        ...
        if not game_active:
            ...
            else:  # 包括 bg_value == WHITE 或 bg_value == "grid"（实际背景白色）
...
while running:
    ...
    # 鼠标移动方向（游戏运行时）
    if game_started and not settings_mode and game_active:
        # 获取鼠标世界坐标
    ...
    if game_started and not settings_mode and game_active:
        ...
        for enemy in enemies:
            ...
            enemy['dir'] = new_dir
            new_enemy_head = (old_enemy_head[0] + new_dir[0] * current_speed,
                              old_enemy_head[1] + new_dir[1] * current_speed)
