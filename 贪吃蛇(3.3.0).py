...
BACKGROUND_STYLE_VALUES = [BLACK, WHITE, "grid"]  # "grid" 现在表示白色/浅灰交错格子
...
def show_full_changelog():
    """显示所有版本的更新日志，可按 ESC 或点击返回按钮退出"""
    # 半透明背景
    log_surface = pygame.Surface((WIDTH, HEIGHT), pygame.SRCALPHA)
    log_surface.fill((192, 255, 240, 220))  # 海沫绿半透明
    ...
    waiting = True
    scroll_offset = 0
    max_scroll = 0  # 简化，不实现滚动，先显示所有内容
    # 如果内容太多，可以加滚动，但这里先简单显示所有，若超出屏幕可通过滚动查看
    # 计算总内容高度
    content_height = 150  # 标题起始y
    content_height += len(ALL_CHANGELOGS) * (30 + 20 * max(len(log) for _, log in ALL_CHANGELOGS))  # 粗略估计
    # 如果内容高度超过窗口，我们允许通过鼠标滚轮滚动，但为简化，暂时只显示部分，加一个提示
    # 这里我们实现简单的滚动（鼠标滚轮）
    content_lines = []
    for ver, lines in ALL_CHANGELOGS:
        ...
        content_lines.append(("spacer", ""))  # 空行

    max_visible_lines = 20  # 大约能显示的行数
...
def draw():
    ...
    if settings_mode:
        ...
        # 设置屏幕背景色（纯色或白色底）
        if bg_value == "grid":
            screen.fill(WHITE)  # 格子背景用白色底，然后绘制格子
        else:
