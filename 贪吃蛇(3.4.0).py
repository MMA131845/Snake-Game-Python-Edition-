...
# ========== 重启游戏（不改变状态） ==========
def restart_game():
    """重置游戏状态，但保持 game_started 为 True（用于暂停菜单的重新开始）"""
...
def draw():
    ...
    if settings_mode:
        ...
        if paused:
            ...
            hint = hint_font.render("↑↓选择  Enter确认", True, LIGHT_GRAY)
        ...
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_ESCAPE:
                if game_started and not settings_mode:
                    ...
                    # 开始界面按ESC可以退出？按需，这里保留原有逻辑：开始界面ESC无作用（或可设置返回？暂时无操作）
                    pass
            elif event.key == pygame.K_SPACE:
                if not game_started and not settings_mode:
                    start_game()
                elif paused:
                    # 暂停时空格可视为确认？为了统一，我们让空格也触发确认
                    # 为简化，不处理，让用户用回车
                    pass
            ...
            if event.button == 1:
                if not game_started and not settings_mode:
                    ...
                    option_y_start = 350
                    for i in range(3):
                        opt_rect = pygame.Rect(WIDTH//2 - 200, option_y_start + i * 60 - 30, 400, 50)
