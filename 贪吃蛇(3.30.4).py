...
ALL_CHANGELOGS = [
    ("v3.30.4", [
        "界面全面升级：复刻 macOS 风格悬停效果",
        "按钮悬停时放大、显示半透明圆角背景及阴影",
        "设置、暂停、结算等所有菜单统一悬停反馈",
    ]),
    ("v3.30.3", [
...
def show_rankings():
    ...
    while waiting:
        ...
        bg_surf.fill((255, 255, 255, 150))
        screen.blit(bg_surf, btn_rect.topleft)
        pygame.draw.rect(screen, GRAY, btn_rect, 2, border_radius=12)
...
def show_name_input_screen():
    ...
    while waiting:
        ...
        pygame.draw.rect(screen, (0, 0, 0, 80), shadow_rect, border_radius=12)
        btn_bg = pygame.Surface((button_rect.width, button_rect.height), pygame.SRCALPHA)
        btn_bg.fill((255, 255, 255, 150))
        screen.blit(btn_bg, button_rect.topleft)
        pygame.draw.rect(screen, GRAY, button_rect, 2, border_radius=12)
...
def show_full_changelog():
    ...
    while waiting:
        ...
        shadow_rect = button_rect.move(3, 3)
        pygame.draw.rect(screen, (0,0,0,80), shadow_rect, border_radius=12)
        bg_surf = pygame.Surface((button_rect.width, button_rect.height), pygame.SRCALPHA)
        bg_surf.fill((255,255,255,150))
        screen.blit(bg_surf, button_rect.topleft)
        pygame.draw.rect(screen, GRAY, button_rect, 2, border_radius=12)
        btn_text = font_button.render(back_button_text, True, BLACK)
...
def draw():
    ...
    if settings_mode:
        ...
        for i, text in enumerate(items):
            display_text = text
            is_hover = (i == selected_setting)
            font_size = 28 if is_hover else 24
    ...
    # ========== 开始界面 ==========
        ...
        if mode_hover:
            mode_color = GRAY
            mode_rendered = mode_font.render(mode_text, True, mode_color)
            shadow_rect = mode_rect.move(3,3)
            pygame.draw.rect(screen, (0,0,0,80), shadow_rect, border_radius=10)
            bg_surf = pygame.Surface((mode_rect.width, mode_rect.height), pygame.SRCALPHA)
            bg_surf.fill((255,255,255,150))
            screen.blit(bg_surf, mode_rect.topleft)
            pygame.draw.rect(screen, (180,180,180), mode_rect, 2, border_radius=10)
        screen.blit(mode_rendered, (WIDTH//2 - mode_rendered.get_width()//2, HEIGHT//2 + MODE_BUTTON_Y))

        # 主按钮
        button_texts = ["开始游戏 (SPACE)", "设置 (S)", "退出 (Q)"]
        button_fonts_sizes = [24, 18, 18]
        button_hover_sizes = [28, 22, 22]
        for i, (y, text) in enumerate(zip(BUTTON_Y_POS, button_texts)):
            screen_y = HEIGHT//2 + y
            is_hover = (hover_button == i)
            font_size = button_hover_sizes[i] if is_hover else button_fonts_sizes[i]
            font = get_font(font_size)
            rendered = font.render(text, True, BLACK if not is_hover else DARK_GRAY)
            btn_width = rendered.get_width() + 40
            btn_height = rendered.get_height() + 20
            btn_rect = pygame.Rect(WIDTH//2 - btn_width//2, screen_y - btn_height//2, btn_width, btn_height)
            if is_hover:
                shadow_rect = btn_rect.move(3,3)
                pygame.draw.rect(screen, (0,0,0,80), shadow_rect, border_radius=12)
                bg_surf = pygame.Surface((btn_width, btn_height), pygame.SRCALPHA)
                bg_surf.fill((255,255,255,150))
                screen.blit(bg_surf, btn_rect.topleft)
                pygame.draw.rect(screen, (180,180,180), btn_rect, 2, border_radius=12)
            screen.blit(rendered, (WIDTH//2 - rendered.get_width()//2, screen_y - rendered.get_height()//2))
    ...
    # ========== 游戏进行中 ==========
        ...
        if not game_active:
            ...
            for i, opt in enumerate(GAMEOVER_OPTIONS):
                is_hover = (i == gameover_selection)
                font_size = 40 if is_hover else 36
                font = get_font(font_size)
                if is_hover:
                    text = "▶ " + opt + " ◀"
                    color = YELLOW
                ...
                if is_hover:
                    btn_width = rendered.get_width() + 30
                    btn_height = rendered.get_height() + 15
                    btn_rect = pygame.Rect(WIDTH//2 - btn_width//2, option_y_start + i * 60 - btn_height//2, btn_width, btn_height)
                    shadow_rect = btn_rect.move(3,3)
                    pygame.draw.rect(screen, (0,0,0,100), shadow_rect, border_radius=10)
                    bg = pygame.Surface((btn_width, btn_height), pygame.SRCALPHA)
                    bg.fill((255,255,255,120))
                    screen.blit(bg, btn_rect.topleft)
