...
TEAM_SIZE = 4
CAPTURE_RADIUS = int(80 * SCALE)       # 占领区域半径
...
teammates = []
enemy_team_data = []
capture_progress = 0          # 占领进度条 0~CAPTURE_TIME
capture_zone = (0, 0)         # 占领区域中心
red_spawn_timers = {}         # 敌方复活计时
SPAWN_DELAY = 3.0             # 敌方重生延迟
...
def restart_game():
    ...
    if game_mode == "team4v4":
        ...
        for i in range(3):
            x = capture_zone[0] - 200*SCALE + random.randint(-60*SCALE, 60*SCALE)
            y = capture_zone[1] + random.randint(-60*SCALE, 60*SCALE)
        ...
        if game_started and game_active and not paused and not gameover_screen:
            ...
            if game_mode == "team4v4":
                ...
                for tm in teammates:
                    ...
                    tm['body'][0] = (tm['body'][0][0] + tm['dir'][0]*BASE_SPEED*0.7, tm['body'][0][1] + tm['dir'][1]*BASE_SPEED*0.7)  # 友方移动速度降低
                    tm['body'].pop()
                    tm['body'].insert(0, tm['body'][0])
                # 处理死亡标记
                # 将移除的敌人加入重生列表（在碰撞中已从列表移除，但这里我们用一个简单的方法：当敌方body被置为(-999,..)时等待重生）
                # 由于我们在碰撞时直接remove，所以需要另外一种方式管理重生。我们可以在生成敌人时赋予id，移除时记录id和体力计时。
                # 为简化，这里让敌方一旦死亡立刻在出生点重生。
                # 替换直接remove为移动至出生点并重置
                # 已在上面修改了移除逻辑，需要调整。现在重新实现：
                # 在碰撞检测中，不再移除，而是将敌方的身体移回出生点并重置计时。
                pass
            else:
                # 经典/淘汰之王模式（保持不变）
                collision_threshold_sq = (current_radius * 2) ** 2
