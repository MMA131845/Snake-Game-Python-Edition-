...
# ========== 游戏版本：v2.15.3 - 按钮位置上移 ==========
VERSION = "v2.15.3"
CHANGELOG = f"""版本 {VERSION} - 按钮位置上移

✨ 新增功能：
- 开始界面按钮整体上移10像素，布局更合理
- 悬停效果与点击区域完全匹配
...
def on_mouse_move(event):
    ...
    if not game_started and not settings_mode:
        # 按钮中心Y坐标（上移10像素）
        button_y_positions = [60, 10, -40, -90]  # 原为[50,0,-50,-100]
        hover_button = None
...
def on_click(event):
    ...
    x = event.x - WIDTH/2
    y = HEIGHT/2 - event.y
    button_y_positions = [60, 10, -40, -90]  # 上移10像素
    buttons = [
        {"func": start_game},
        {"func": enter_settings},
        {"func": restart},
