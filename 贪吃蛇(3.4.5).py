...
def draw():
    ...
    if settings_mode:
        ...
        text_rect.center = (BACK_BUTTON_POS[0] + 50, BACK_BUTTON_POS[1] + 20)  # 原按钮宽100高40
...
while running:
    ...
    if game_started and not settings_mode and game_active and not paused:
        current_speed = get_current_speed()          # 玩家速度（可能加速）
        enemy_speed = get_base_speed()                # 敌人速度（基础速度）
        current_radius = get_segment_radius()
