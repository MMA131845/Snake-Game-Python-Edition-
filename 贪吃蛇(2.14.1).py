...
# ========== 游戏版本：v2.14.1 - 修复鼠标点击 ==========
VERSION = "v2.14.1"
CHANGELOG = f"""版本 {VERSION} - 修复鼠标点击

✨ 新增功能：
- 开始界面支持鼠标点击按钮（开始游戏、设置、重新开始、退出）
- 保留键盘快捷键（空格、S、R、Q）作为备用
- 所有界面文字改为微软雅黑字体
- 设置界面颜色选项显示中文
- 可自定义蛇头颜色、蛇身颜色和背景样式
- 开始界面标题后显示版本号
...
def on_click(event):
    ...
    x = event.x - WIDTH/2
    y = HEIGHT/2 - event.y
    # 按钮位置定义（与draw中的文字位置对应）
    button_y_start = 50  # "开始游戏"的y坐标
    button_spacing = 50  # 按钮之间的间距
    button_width = 300   # 估算的按钮宽度
    button_height = 40   # 估算的按钮高度
    buttons = [
        {"func": start_game, "y": 50, "key": "space"},   # 开始游戏
        {"func": enter_settings, "y": 0, "key": "s"},    # 设置
        {"func": restart, "y": -50, "key": "r"},         # 重新开始
        {"func": quit_game, "y": -100, "key": "q"}       # 退出
    ]
    for btn in buttons:
        if btn["y"] - button_height/2 <= y <= btn["y"] + button_height/2 and \
           -button_width/2 <= x <= button_width/2:
            btn["func"]()
            break
...
def draw():
    ...
    if settings_mode:
        ...
        if int(time.time() * 2) % 2 == 0:
            writer.goto(0, 50)
            writer.write("开始游戏 (SPACE)", align="center", font=("Microsoft YaHei", 24, "bold"))

        # 设置选项
        writer.goto(0, 0)
        writer.write("设置 (S)", align="center", font=("Microsoft YaHei", 18, "normal"))

        # 重新开始选项
        writer.goto(0, -50)
        writer.write("重新开始 (R)", align="center", font=("Microsoft YaHei", 18, "normal"))

        # 退出选项
        writer.goto(0, -100)
        writer.write("退出 (Q)", align="center", font=("Microsoft YaHei", 18, "normal"))
