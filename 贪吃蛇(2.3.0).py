...
# ========== 游戏版本：v2.3.0 - 贪吃蛇大作战风格 ==========
VERSION = "v2.3.0"
CHANGELOG = f"""版本 {VERSION} - 贪吃蛇大作战风格

✨ 新增功能：
- 深色网格背景（#1a1a1a + 灰色网格线）
- 地图扩大至 5000×4000，摄像机跟随
- 鼠标移动控制方向
- 敌人AI主动觅食，持续生成（最多10个）
- 敌人死亡掉落密集红色大食物
...
WARNING_CIRCLE_COLOR = "white"  # 预警圆圈用白色（深色背景更明显）
...
def draw():
    ...
    if not game_started:
        ...
        writer.color(TEXT_COLOR)  # 恢复为白色（用于游戏画面）
        ...
        # 绘制网格线（只绘制视野范围内的，提高性能）
        pen.color(GRID_COLOR)
        pen.pensize(1)

        # 计算网格起始和结束（取整到GRID_SIZE的倍数）
        ...
        x = start_x
        while x <= end_x:
            sx, _ = world_to_screen(x, 0)  # 只需转换x坐标
            pen.penup()
